...
- In-portal invite accept requires the member's 4-digit sign-off PIN, using the SAME server-side gate as document sign-off (lockout, pin_not_set, attemptsRemaining) and the shared useSignOffFlow hook on the frontend (set-PIN and forgot-PIN paths come free). New request bodies added to the openapi spec need an explicit re-export in lib/api-zod/src/index.ts or tsc --build fails with TS2308.
