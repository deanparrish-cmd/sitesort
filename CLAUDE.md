...
- **2026-07-30 (2) — session-opener housekeeping:** CLAUDE.md was over the 30k trim threshold (31.6k chars) — moved features #94/#95/#96 full detail to CLAUDE_ARCHIVE.md, kept condensed one-liners in place, now 28.3k chars. `git pull` correctly left alone (see below) — local/origin have diverged 140/80 commits, expected for this repo's one-way sync model. 8 files show as locally modified (`projects.ts`, dashboard/invoices/projects pages, generated api-client/zod/openapi files) — not yet diagnosed this session.
