...
- **2026-06-28 — "Check your email" screen UX (Feature #60 follow-up; pushed, NOT yet deployed):** Improved the post-signup verification screen (`register.tsx`) for users who can't find the email: (1) spam line now reads "check your spam or junk folder — and mark it 'not spam'…"; (2) **rate-limited resend** — replaced the one-shot button with a **45s frontend countdown** ("Resend available in Ns", disabled) backed by a silent **30s per-email in-memory throttle** in `/auth/resend-verification` (`resendThrottled()`; still always returns `{success:true}` so anti-enumeration is preserved); (3) **"Wrong email? Go back and edit"** link returns to the details form with the typo'd address preserved (`editEmail()` — form stays mounted so values persist). Verified: ✅ typecheck; real browser on single-origin `:8080` (9/9 checks, 0 console/page errors, screenshot); backend throttle confirmed via DB token-rotation (resend #1 rotates token, immediate #2 unchanged = throttled); all test data cleaned. Pushed `main → a98f5b26`. ⏳ **TODO: PD must click Publish** (carries #60 too if not yet live).
- **2026-06-27 — Real email verification on registration (Feature #60; NOT yet deployed):** `/auth/register` now creates the account **`emailVerified:false`** with a 32-byte token + 24h expiry, sends the **verification email** (not welcome), and **issues NO JWT** — returns `{requiresVerification, email}` (new `RegisterResponse` schema; register 201 no longer shares `AuthResponse`; codegen regenerated). Welcome email now fires **only after** verification (existing `/auth/verify-email` handler, unchanged). **`ensure-schema.ts`** gained idempotent `ADD COLUMN IF NOT EXISTS` for `users.email_verified`/`email_verification_token`/`email_verification_expiry` (register now writes the token cols; prod DB had no guarantee). Frontend: `register.tsx` shows a **"Check your email"** screen (MailCheck icon, email, Resend button via `/auth/resend-verification`, Log in link) instead of auto-login+Stripe; `verify-email.tsx` routes success/error to **Log in**. Login page already handled `email_not_verified` + resend (untouched). **New flow:** register → check email → click link → verified + welcome → log in → existing CheckoutGate takes card → app. Verified end-to-end on single-origin `:8085` against real DB (register→201 no token + unverified row; pre-verify login→403; verify→cleared+verified; post-verify login→200) AND in browser (check-email screen renders, 0 console/page errors); test data cleaned. ✅ typecheck. ⏳ **TODO: PD must click Publish to deploy** (GitHub pushed `main → 296490d6`).
