...
102. **On-hire plant stays on dashboard, overdue in red** — new `GET /api/plant-items/on-hire` + dashboard "Plant on hire" card (no date filter; leaves only when marked off-hired; overdue first). Root cause of the tester's report: dashboard had NO plant section and the weekly plant report's week filter dropped overdue items (now ... *(full detail in CLAUDE_ARCHIVE.md)*
103. **QR site sign-out + live on-site register** — one `site_checkins` row per in/out cycle (`checked_out_at/by`, `checkout_note/method`); public `GET /site/:token/status` + `POST /site/:token/checkout`; same-day double sign-in 409s; `OnSiteRegister` (project Check-Ins tab + `/checkins`) shows who is on site, flags previous-day open ... *(full detail in CLAUDE_ARCHIVE.md)*
104. **Photos inside a daily site report** — `photos.daily_report_date`; `GET/POST /projects/:id/daily-reports/:date/photos`; `DailyReportPhotos` in the report dialog; `FileDropZone` gained `multiple` mode. Same row shows in the photo library (overlay tag "Daily report <date>"), category `progress`. Drag-drop of 2 files verified in ... *(full detail in CLAUDE_ARCHIVE.md)*
105. **Portal report photos + public on-site count** — `DailyReportPhotos`/`FileDropZone` reused in the portal (today's report editable per the same lock/submitted/privacy rules as PATCH; history + shared reports read-only); portal-scoped `POST /portal/daily-report/:date/photos/upload` (portal JWTs are blocked from `/api/upload`; ... *(full detail in CLAUDE_ARCHIVE.md)*
106. **QR sign-in/out: remembered device, name lookup, near-match, company autocomplete** — `POST /site/:token/checkin` returns a signed per-project `deviceToken` (JWT, 180d, nothing new stored server-side) kept in localStorage; new public `GET /site/:token/device|who|companies`, `POST /checkout` accepts `{checkinId}`. `who` lists ... *(full detail in CLAUDE_ARCHIVE.md)*
107. **Permits-tab "N overdue" pill now deep-links** (closes the standing dead-badge gap) — many overdue permits: filtered list w/ "Show all" banner (folders auto-open); exactly one: opens it (manager edit dialog, else its certificate, else the filtered list). Browser-verified both cases.
108. **Check-in notifications open a detail; identity-linked check-ins; check-in near-match; sign-outs in the feed** — `CheckinNotificationDialog` (dashboard feed + `/notifications`) via `GET /notifications/:id/checkin` (photo w/ lightbox, times, Still on site, link to project Check-Ins tab; blocked shows what was typed + why). ... *(full detail in CLAUDE_ARCHIVE.md)*
109. **Site check-in QR / URL restricted to Admin + Project Manager (enforced server-side)** — `GET/POST/DELETE /projects/:id/qr-codes` now 403 for everyone but company `admin`/`project_manager` (`requireQrManager` in qr.ts; portal-scoped tokens were already contained to `/api/portal`); `/portal/site-board` no longer returns `qrToken` (spec + codegen updated); portal Site Board QR card removed; dashboard hides the Site Board-tab QR, `/qr` page + sidebar item, the Check-Ins tab "View Site Board" link and the Share modal site link for non-managers. Only remaining check-in URL surfaces: the manager-only ones + the public `/site/:token` page itself. Tests: `tests/qr-code-access.test.ts` (5). Portal Home/Site Board still show the other board content (project, site manager, permits, documents, trades, pins, events).
110. **Public suggestions show initials only** — 'Did you mean' at check-in AND sign-out now show first name + surname initial + company (`publicLabel` in qr.ts, e.g. "Amy P, Amy I Cloud"). Check-in suggestions no longer return a name to fill in: they return an opaque signed `matchToken` (kind site-match, holds only the registered record's key, 10 min) that the check-in POST accepts as `matchToken`; the server then checks in the confirmed record under its own spelling. Client keeps what was typed and shows a "Checking in as ..." chip. Tests updated (+1 forged-token case).
