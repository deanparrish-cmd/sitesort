...
- **2026-07-11 (later) — PROD 502-on-login FIX + hardening (PUSHED, AWAITING PUBLISH):** Root cause = the pg `Pool` (`lib/db`) had **no `'error'` listener**, so a dropped idle connection (managed PG closing idle conns / network blip) surfaced as an unhandled EventEmitter error → **process crashed** → Replit restart → **intermittent 502** (login was 200 when checked → crash-restart). **Fixes:** `pool.on('error')` + pool timeouts/`checkDbConnection()` (`lib/db`); process-level `unhandledRejection`/`uncaughtException` (log + stay alive) in `index.ts`; global Express error handler + **JSON 404 for unmatched `/api/*`** (was SPA-html 200, masking failures) in `app.ts`; **real `GET /api/health`** (DB probe → 200/503); startup DB check + `ensureSchema` timeout guard + `0.0.0.0` bind. Verified locally. **NEXT:** PD Publishes → prod-verify `/api/health` 200 + login.
- **2026-07-11 — F5 Daily Site Reports hub (#62) — DEPLOYED+Published+prod-verified:** company-wide hub + editable site diary + voice-to-text (see #62). Prod-verified: `GET /api/daily-reports` 200 (109 rows), PATCH round-trip + real-browser clean.
