...
- **RESOLVED 2026-06-23 (session 3):** Live expiry-reminder email send **confirmed on prod** (the item open since session 2). Fired `POST /api/test-email` `{"template":"permit"}` → `200 {"ok":true}` to two inboxes (dean.parrish@me.com + amy-parrish@hotmail.co.uk), both **received**. Proves `RESEND_API_KEY` is set in prod + Resend accepts + the `sendPermitExpiryEmail` template renders/sends end-to-end. **Scheduled job also verified end-to-end:** created a test permit expiring 2026-06-30 (7d out, milestone "7") on the Riverside demo project, Responsible Person = Amy (amy-parrish@hotmail.co.uk); after a republish the daily job's 30s boot run sent the reminder and Amy **received** it → the automated path confirmed, not just the manual `/api/test-email` route. NB the job runs 30s after boot + every 24h, so a newly-added in-window item needs a republish (or the 24h tick) to be picked up. Test permit `193f7158…` **deleted** after verification (DELETE 200, confirmed gone) — Riverside project back to original state; its `expiry_reminder_logs` "7" row is harmless residue (clear by `entity_id` to re-test).
