...
- **NEXT SESSION TODO — PD test backlog (resume here):** Working PD's prioritised backlog. **Done + DEPLOYED+live:** B1, B2, F1 Phase 0+1, F1 Phase 2 (permits). **Done, pushed to GitHub, NOT yet Published:** F1 Phase 3 (insurance, `f62ec479`), F2 (close-out — see 2026-06-28 entry; commit `f741ccf`, **not yet pushed**). **NEXT = F3** alphabetical drawing revisions. Then remaining: **F4** group MS/Permit/Safety under one H&S tab, **F5** Daily Site Reports hub, **F6** subbie/merchant docs at setup, **F7** Site Board live on-site count, **F8** Timeline programme link (decision-first), **F9/F10** spikes only. (Optional follow-ups: F1 P3 — assignee/OVERDUE on Compliance Centre insurance rows + Assign-to/Due-by in the insurance upload flow; F2 — surface readiness/close-out on Compliance Centre or dashboard.)
- **2026-06-28 — F2: project close-out / handover (committed `f741ccf`, NOT yet pushed/deployed):** Manager-gated **"Close-out" tab** on project detail. **Schema:** new append-only **`project_closeouts`** (who/role/note/ip/ua/timestamp, cascade) in `ensure-schema.ts`. **Backend (`routes/closeout.ts`):** `GET /projects/:id/closeout` → 4 advisory readiness checks (open snags, subcontractor insurance, expired permits, pending doc sign-offs) + current record; `POST` reuses the **document sign-off PIN mechanism** (bcrypt + `pin-attempts` lockout; `pin_not_set`/`invalid_pin`+attemptsRemaining/`too_many_attempts`), writes audit row + sets `status=complete` in one txn; `POST .../reopen` → active (audit kept). Manager-only (admin/PM), tenant-scoped. **Frontend (`projects/detail.tsx`):** checklist (green/amber per item) + "Sign off & mark Complete" PIN dialog (mirrors doc sign-off incl. just-in-time PIN setup + optional handover note) + completed state w/ record + Re-open. Raw fetch + local types (matches the page's other sub-resources — **no OpenAPI/codegen**). Verified typecheck + API (incl. wrong-PIN→401, double→400, non-manager→403, audit retained) + browser `:8099` (10/10); demo fully reverted. ⏳ **TODO: push + PD Publish.**
- **2026-06-28 — F1 Phase 3: insurance cert accountability (pushed `main → f62ec479`, NOT yet Published):** Mirrors Phase 2 for subcontractor insurance. `insurance_records` + `assigned_to_user_id` (FK users) + `due_date` (idempotent in `ensure-schema.ts`). `serializeInsuranceRecords` (subcontractors.ts) adds assignee id/name + dueDate + derived `overdue = isOverdue(dueDate,!!archivedAt)`; status via shared `expiry.ts` (`active`→`valid`) in subcontractors.ts + team.ts. New tenant-scoped IDOR-safe **`PATCH /subcontractors/:id/insurance/:recordId`**; POST insurance accepts assignee/dueDate. Spec+codegen regenerated (`UpdateInsuranceRequest`/`updateInsuranceRecord`). Frontend contacts directory: rows show assignee + due-by + **OVERDUE**; manager-gated "Insurance Accountability" dialog. Verified typecheck + API + browser (6/6).
