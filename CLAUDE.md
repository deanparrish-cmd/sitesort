...
- **2026-07-14 (both DEPLOYED+prod-verified):** (1) **Per-person Team Portal invites** = Feature **#63** (portal-only for everyone; new `people` table). (2) **Messages badge/list mismatch** — badge counted DMs across ALL companies vs company-scoped list (#57) → shared `unreadDmFilter`/`isUnreadDmRow` (`routes/messages.ts`) + `sitesort:messages-read` badge refresh (badge = DMs only, not channels).
- **Opportunity:** point an uptime monitor at `GET /api/health`. **Backlog** (scope w/ PD): F6–F10 (see PD test backlog below).
- **2026-07-11 — PROD 502 FIX (DEPLOYED):** pg `Pool` lacked an `'error'` listener → restart loop → 502; fixed + `GET /api/health`. Archive.
- **PD test backlog:** Done+DEPLOYED: B1–2, F1–F5, Team Portal (#61). Remaining (confirm scope w/ PD, per F2–F4 pattern): **F6** subbie/merchant docs at setup, **F7** Site Board live on-site count, **F8** Timeline programme link, **F9/F10** spikes.
