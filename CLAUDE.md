...
87. **Activity-entry deep-links + daily reports join Team Portal sharing** — audited every activity/notification entry referencing an item and every Share entry point (both explicitly requested); fixed 3 non-clickable-activity gaps (daily report document-activity rows, PM oversight Activity feed's "(opened an item)" placeholder, dashboard/notifications' `document_uploaded` handler) via a new shared `lib/deep-link.ts`, and closed the one real Share-dialog gap (`daily_report` excluded from `ShareModal`'s Team Portal gate) end-to-end — new `/portal/shared` daily-report aggregation + view-log endpoint, scoped to the site diary only per an explicit user decision, never the auto-collated check-ins/document-activity/photos. **Built+verified locally** (2026-07-23): `typecheck` clean, `check:layout` 96/96, full functional round-trip + browser screenshots confirming both fixes. Not yet pushed/published — auto-swept into a Replit checkpoint commit (`460737c`), not a Publish. *(full detail in CLAUDE_ARCHIVE.md)*
