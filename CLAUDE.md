...
- **2026-06-23:** Built the dashboard outstanding-invoices widget (Feature #58 — the DECIDED TODO from 2026-06-19 session 3). Added to `pages/dashboard/index.tsx`: local helpers (`fmtAmount`/`fmtDate`/`daysUntilDue`/`InvoiceStatusBadge`), `shareInvoice`/`moveToInvoice`/`movingProject` state, `markInvoicePaid`/`moveInvoiceToProject` handlers, `outstandingInvoices` memo (unpaid, overdue-first, top 5) + `outstandingCount`. Widget card sits between Needs Attention and Active Projects with Open/Share/Mark Paid labeled pills, plus a move-to-project Dialog and a `ShareModal`. `Invoice` type extended with `attachmentUrl`/`projectId`. Dropped Mark Unpaid (list is unpaid-only) — kept the helper out to avoid dead code. Typecheck passes. **Verified** in real browser (rebuilt frontend, Express single-origin on :8080, logged in paul@acme.com): widget renders the one overdue "test" invoice (EUR 350.00) with Open/Share/Mark Paid; clicking Mark Paid flipped status→paid and removed the row (no move dialog because the invoice was already project-linked → toast branch), zero console/page errors. Restored the test invoice to `pending` afterwards so demo data is unchanged. ✅ DEPLOYED — committed + pushed to GitHub (verify-push: 12/12 signatures on main), published to Replit, and verified live on www.sitesort.co.uk (logged in paul@acme.com: widget renders the overdue "test" invoice EUR 350.00 with Open/Share/Mark Paid pills, live `/api/invoices` = 1 unpaid, zero console/page errors). NOTE: CLAUDE.md is now ~29k chars — trim oldest session logs to CLAUDE_ARCHIVE.md next session before it crosses 30k.
- **2026-06-19 (session 3):** Bare-icon → labeled-pill UI consistency pass on `projects/detail.tsx`. Converted every icon-only action button (Open/Share/History/Edit/Notes — previously icon + `title=` only) to labeled rounded-full pills (`inline-flex items-center gap-1.5 px-2.5/px-3 py-1/py-1.5 rounded-full border ... text-xs/sm`, all `type="button"`). Spots: **Documents** desktop table (Open/History/Edit/Share); **Finances & Expiry** tab → Document Status (Open/Share) + Permit Expiry (Open conditional on `documentUrl` + Share); **Overview** daily-note cards (Open/Share); **Check-ins** tab cards (Share); **Team** tab contact cards (Notes [subcontractor-only] + Share). Action containers set to `flex-wrap` so wider pills reflow. **Findings (not bugs):** invoice pills (Open/Share/Mark Paid/Mark Unpaid, with project-move logic) were *already* fully implemented on `invoices/index.tsx` AND the project Finances invoice rows; zero empty `onClick={}`; all `pointer-events-none` hits are legitimate/non-blocking. **Verified** via real browser (built frontend + ran Express on :8080 single-origin, logged in as paul@acme.com, drove Riverside Apartments project): Documents table, Finances Document Status + Permit Expiry, and Team Share pills all render correctly, no console/page errors. Typecheck passes. **TODO next session (DECIDED — build it):** add a **dashboard invoice widget** to the main dashboard (`/`, `pages/dashboard/index.tsx`). Currently the dashboard only feeds invoices into the calendar + an "overdue" attention link — there are no invoice rows. Build an invoice list/cards widget with the same labeled pills used elsewhere: **Open**, **Share**, **Mark Paid** (amber, moves invoice to its project + status=paid), **Mark Unpaid**. Reuse the existing pill markup + `markPaid`/`markUnpaid` patterns from `invoices/index.tsx` and the project Finances tab (`projects/detail.tsx`) — invoices are already fetched on the dashboard (`const [invoices, setInvoices]`, `GET /api/invoices`), so no new API needed. Team **Notes** pill is subcontractor-gated and couldn't be shown visually (demo projects have no subcontractor members) — verified by code/markup parity with the Share pill, not by screenshot. ✅ COMMITTED (pending deploy).
