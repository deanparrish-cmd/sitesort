...
- **2026-07-14 — Messages unread badge/list mismatch FIX (committed+pushed, awaiting Publish):** Badge showed 1 but no conversation showed it. Root cause = `GET /api/messages/unread-count` counted unread DMs across **ALL companies** while `/messages/conversations` is **active-company**-scoped (Feature #57 multi-company) — a DM in another of the user's companies inflated the badge but never appeared in the list. Fix: shared **`unreadDmFilter()`** (SQL) + **`isUnreadDmRow()`** (JS) in `routes/messages.ts` (`companyId`+`recipientId`+`readAt IS NULL`), used by BOTH badge + list so they can't drift. Also: opening a DM clears its unread pill locally and fires a `sitesort:messages-read` window event (`lib/message-events.ts`) → sidebar re-fetches badge instantly (these views use fetch+poll, not React Query). NB: badge counts DMs only, not channel unread — possible follow-up.
- **Opportunity (not a bug):** point an uptime monitor at `GET /api/health` (200/503 + db status). **Backlog** (scope w/ PD first): F6–F10 (see PD test backlog below).
- **2026-07-11 — PROD 502-on-login FIX (DEPLOYED):** pg `Pool` had no `'error'` listener → restart loop → 502. Fixed + added `GET /api/health`. Detail in CLAUDE_ARCHIVE.md.
