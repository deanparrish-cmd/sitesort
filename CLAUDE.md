...
- **Next-session notes (2026-07-11 wrap):** all today's work DEPLOYED+prod-verified, tree clean, nothing WIP. **Opportunity (not a bug):** point an uptime monitor at the new `GET /api/health` (200/503 + db status). **Backlog** (scope w/ PD first) — see PD test backlog below: F6–F10. Housekeeping: CLAUDE.md near 30k ceiling — archive a bullet before adding.
