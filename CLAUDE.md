...
- **NEXT SESSION TODO (from 2026-06-23 session 2):** Confirm a real expiry-reminder email actually sends on prod — *not yet observed*. Live finding: all 5 current prod expiring permits/certs are **69–88 days past expiry**, so they're outside both the 1–30d pre-expiry window AND the 0–6d post-expiry grace → `milestoneFor()` returns null and the job correctly sends nothing (intended: no nagging about long-dead items). To observe a send: add a test permit with expiry ~7 days out, set Responsible Person to a reachable inbox (user was walked through adding `dean.parrish@me.com` as a team member + linking to the project), ensure that user's email notifications are on, then **republish** (job runs ~30s after boot, else next 24h) and check the inbox. Also re-confirm `RESEND_API_KEY` is set in prod. De-dup note: each (permit, milestone) sends once — to re-test the same milestone, clear its row in `expiry_reminder_logs`.
- **2026-06-23 (session 2):** Feature #59 — expanded expiry email reminders. Permits + insurance certs now email at **30/21/14/7/1 days** (once each) then **daily for 7 days once expired** (was only ~7d/~1d, with no de-dup so it could re-send). New `expiry_reminder_logs` table (`entity_type, entity_id, milestone`, unique) + boot migration in `ensure-schema.ts`. Rewrote `permit-reminders.ts`: `milestoneFor(daysLeft)` buckets to the smallest threshold ≥ daysLeft (no burst for late-added items) / `exp-0…exp-6` for expired; `claimMilestone` uses `onConflictDoNothing` so each milestone sends exactly once. Now also skips `archivedAt` records. Extended `sendPermitExpiryEmail` for expired wording (insurance already had it). Dashboard/calendar already show ≤30d-incl-expired via compliance API — no frontend change. Verified: 24/24 unit cases on `milestoneFor`/`daysUntil`; DB de-dup confirmed against dev DB; typecheck passes. ✅ DEPLOYED — committed + pushed to GitHub (verify-push OK on main), published to Replit, live boot verified on www.sitesort.co.uk (health 200, login works → `ensureSchema` ran cleanly so the new table deployed safely; compliance feed = 3 expiring permits + 2 insurance; dashboard renders "Expiring Soon"=5 + Needs Attention expired items, zero console/page errors). The reminder emails themselves fire on the server's daily schedule and need `RESEND_API_KEY` in prod — verified by logic/de-dup/templates, not by an observed live send.
