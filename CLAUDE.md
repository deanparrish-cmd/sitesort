...
- **2026-07-11 (later) — PROD 502-on-login FIX + hardening (DEPLOYED+prod-verified):** Root cause = the pg `Pool` (`lib/db`) had **no `'error'` listener**, so a dropped idle connection (managed PG closing idle conns / network blip) surfaced as an unhandled EventEmitter error → **process crashed** → Replit restart → **intermittent 502** (login was 200 when checked → crash-restart). **Fixes:** `pool.on('error')` + pool timeouts/`checkDbConnection()` (`lib/db`); process-level `unhandledRejection`/`uncaughtException` (log + stay alive) in `index.ts`; global Express error handler + **JSON 404 for unmatched `/api/*`** (was SPA-html 200, masking failures) in `app.ts`; **real `GET /api/health`** (DB probe → 200/503); startup DB check + `ensureSchema` timeout guard + `0.0.0.0` bind. Prod-verified post-Publish: `/api/health` 200 db:up, `/api/*` JSON 404, login 200.
