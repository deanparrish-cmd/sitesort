...
66. **Real invite emails (Resend) + existing-account portal join** — invites auto-send via Resend (`lib/invite-email.ts`, never throws; delivery state on `project_invites` + rate-limited resend 1/5min); expired-link page; **cross-company invitee joins with existing login** (`grantOnly`→`requiresLogin`, NO link-based session bypass); `DELETE /subcontractors/:id` + orphan portal-user cleanup. **DEPLOYED+prod-verified** (9/9 e2e). *(full detail in git history)*
...
- **2026-07-19:** Found commit `1a49a54` (off-workflow "Replit Agent" session, 2026-07-17) shipped an undocumented feature set an hour before that day's Publish. Reconciled: added **#72**, corrected **#67** to DEPLOYED (superseded by #72's 30-day inactivity policy). Fixed a real typecheck break the commit left behind (`lib/api-zod` tsconfig missing `"dom"` lib; `UploadPortalMyDocumentBody` ambiguous duplicate export) — `pnpm run typecheck` now clean. No deploy needed, #72 already live (401-not-404 probes). **Lesson: cross-check `git log` against this file periodically — it can drift behind off-workflow work.**
...
- **PD test backlog**: Remaining: **F7** Site Board on-site count, **F8** Timeline link, **F9/F10** spikes. **Opportunity:** uptime monitor on `GET /api/health`.
- **Infra facts:** GitHub push ≠ deploy; prod = Replit **Publish** (builds from workspace snapshot). Apex+`www` → `34.111.179.208`. Workspace Stripe env is **LIVE** (`sk_live`). Verify via `.claude/skills/browser-check`; rollout lags ~1-3 min.
