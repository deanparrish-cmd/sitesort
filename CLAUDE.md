...
- `artifacts/sitesort/src/pages/projects/index.tsx` — safety issue modal (project picker, voice description, zone, optional photo → `POST /api/projects/:id/photos` category `safety_concern`); permit modal (project picker, permit type from 9 common types, voice description, responsible person from team, start/expiry dates → `POST /api/projects/:id/permits`); team users loaded from `/api/messages/users`; `?safety=1` and `?permit=1` param detection; both modals auto-select project if only one active
- `artifacts/api-server/src/routes/photos.ts` — `photoUrl` made optional; stored as `null` when not provided
