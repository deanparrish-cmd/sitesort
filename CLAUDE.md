...
- **NEXT SESSION TODO — PD test backlog (resume here):** Working PD's prioritised backlog (full prompts in 2026-06-23 session-3 chat). **Done + DEPLOYED+live:** B1, B2, F1 Phase 0+1 (see entry below). **NEXT = F1 Phase 2 (permits) + Phase 3 (certs)**, following the *exact* Phase-1 pattern — user is reviewing the Phase-1 convention on the Site Issues tab first. F1 decisions locked: `dueDate` = separate action deadline (≠ legal expiry); assignee = internal team **user** (for permits **reuse `responsibleUserId`**, add an editable/reassign UI + `dueDate`; insurance/`insurance_records` gets new `assignedToUserId`+`dueDate`); **consolidate the 5+ scattered expiry-status derivations into one shared helper** alongside `lib/accountability.ts` (`isOverdue` lives there). Add new cols via `ensure-schema.ts`. Then remaining: **F2** close-out+sign-off (snags/safety, composes on F1, reuses upload mechanism), **F3** alphabetical drawing revisions (A/B/C/P01), **F4** group Method-Statement/Permit/Safety under one H&S tab, **F5** Daily Site Reports hub (check F7 sign-in data first), **F6** subbie/merchant docs at setup, **F7** Site Board live on-site count (investigate Site Board first), **F8** Timeline programme link (decision-first, no code yet), **F9/F10** spikes only (do not build).
...
- **Issues discovered this session (for next session):** (1) Expiry-status derivation duplicated in **5+ places with inconsistent thresholds** (`permits.ts` exact-day, `compliance.ts` 7/30d, two frontend badges 7d, reminder job 30/21/14/7/1) → consolidate during F1 ph2-3. (2) `permits` PATCH route exists end-to-end but the **frontend never calls it** (no edit/reassign UI; the id is dropped client-side, only `responsibleName` kept) → wire in F1 ph2. (3) Standalone `/issues` page is still **routed but orphaned** from sidebar nav (#53 moved Site Issues into projects) — kept in sync this session, decide later whether to retire it. None are blockers.
- **Expiry reminders — fully verified live (closed):** Made the daily job observable (`permit-reminders.ts` — warns on missing `RESEND_API_KEY`, per-run `ReminderStats` `scanned/noMilestone/notifyOff/deduped/sent`, per-send breadcrumb; `9e589b4`). Then confirmed real send on prod: `POST /api/test-email` `{"template":"permit"}` → `200`, received at 2 inboxes; AND the *scheduled* job — a 7-day test permit (Responsible = Amy) emailed on the next boot run and was received. Job runs 30s after boot + every 24h, so a new in-window item needs a republish to fire. `POST /api/test-email` (mounted) is the fastest live-send check. Test permit deleted; its `expiry_reminder_logs` "7" row is harmless (clear by `entity_id` to re-test).
- **2026-06-23 (session 2):** Feature #59 — expanded expiry email reminders (full detail in features list #59): 30/21/14/7/1 days then daily for 7 days once expired; `expiry_reminder_logs` table + ensure-schema de-dup; `permit-reminders.ts` `milestoneFor` bucketing + `claimMilestone`. ✅ DEPLOYED + live (send later confirmed — see "Expiry reminders" entry below).
...
- **2026-06-19 and earlier:** See CLAUDE_ARCHIVE.md.
