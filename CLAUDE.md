...
93. **Invoice Allocate + scoped Share (financial-data-safe)** — proper Allocate/Re-allocate/Un-allocate row action; closed a real zero-role-gating gap on `PATCH /invoices/:id`/`POST /share-logs`; secure in-app deep links replace raw attachment URLs in Share. **DEPLOYED+prod-verified** (2026-07-27). *(full detail in CLAUDE_ARCHIVE.md)*
...
96. **Portal navigation cleanup — hamburger removed, no duplication** — continuation of #94's redesign. **Duplication audit**: every one of the 8 tile-eligible destinations (Messages/Shared/My documents/Site Board/Permits/Site Issues/Plant & Materials/Daily Report) was reachable from BOTH a Home tile AND the hamburger sidebar's `SECTION_NAV` list — the sidebar simply duplicated the tile grid. Settings/Help/Log out/profile were sidebar-only, not duplicated but hidden behind the hamburger. **Fix**: `layout.tsx`'s hamburger+sidebar drawer removed entirely, replaced with one unified header (logo+project name, no `md:`/mobile split needed any more) — `SECTION_NAV` is now pure data (label/icon/permission), not a rendered nav list. Every destination lives in exactly one place: the 5 everyday sections stay individual tiles; **Site Issues/Plant & Materials/Daily Report** consolidated behind one new conditional "Site Tasks" tile (`section.tsx`'s `SiteTasksView`, new `/portal/site-tasks` route) — absent entirely with zero permissions, shows only the granted ones (never all three greyed out) with one permission; **Settings/Help/Log out/profile** consolidated onto one "Settings" tile → the Settings page, which gained a profile card (moved from the old sidebar footer), a "Help & User Guide" link row, and a Log out button. `scripts/src/check-layout.ts` extended with the new route. **Verified**: `typecheck` clean, `check:layout` 104/104; live headless-browser pass confirmed zero hamburger elements, a worker with all 3 permissions sees exactly one "Site Tasks" tile opening to all 3 items, one permission → exactly 1 item, zero permissions → no tile at all, Settings page shows profile+Help+Log out. Test fixtures archived (real footprint) after.
...
- **2026-07-30 — shipped #94/#95/#96** (portal visual redesign; check-in insurance split-source + pinned-doc CAD-open fix; hamburger removed + nav-duplication cleanup). Published, prod-verified. **Standing gotcha:** local api-server on `:8080` is a plain `node dist/index.mjs` left running, NOT auto-restarted — rebuild + kill+restart after backend changes before trusting `check:layout`/manual checks.
