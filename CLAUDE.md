...
- **2026-07-25 (2) — shipped #90, three portal fixes** (nav/permission gating, site-manager logic, submit→view flow — see #90). Published, prod-verified.
- **2026-07-25 (1) — fixed the recentActivity gap flagged at end of 2026-07-23 session:** shipped **#89** — see above. `typecheck` clean, verified via API + browser screenshot. Also noted: local `git pull` fails — `main`/`origin/main` diverged (92 vs 65 commits), consistent with `origin` being a synthetic API-pushed mirror, not real shared history; left unresolved pending user decision, use `push-robust.ts` to push.
