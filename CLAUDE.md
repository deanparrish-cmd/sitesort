...
- **2026-07-03 — Team Portal (Feature #61) DEPLOYED+Published ✅:** see #61 for the full feature. STEP-0 investigate-first; PD confirmed "option 1" (dedicated `/api/portal` namespace, one full pass, proposed section mapping). typecheck + both builds green; backend E2E via curl proved invite→accept→reads→containment-403s→activity-feed→revoke; portal pages browser-clean. Committed `6f95ea4`+`7ab6f24`, pushed to GitHub `main → 89a647fb` (16/16 sigs verified). **PD Published** — prod-verified: `/api/portal/invite/x` → 410 JSON (route live + `project_invites` table exists → prod `ensureSchema` ran) and prod `/portal/login` renders clean (0 console errors). NEXT = optional F5 (Daily Site Reports hub).
- **2026-06-29 session wrap:** frontend performance pass (entry below) — **DEPLOYED+Published** (`main → 16bc1bab`). typecheck+build clean, browser-verified desktop/tablet/mobile, 0 errors. Tree clean.
- **2026-06-29 — frontend performance pass (DEPLOYED ✅, `main → 16bc1bab`, Published):** code-splitting/lazy routes (`App.tsx` `React.lazy`+`Suspense`; 2.7 MB monolith → 88 chunks), `react-vendor` `manualChunks` (gotcha: DON'T name-chunk route-exclusive libs like recharts — drags shared helpers eager), images→WebP (hero 1.3 MB→44 KB etc., PNG originals kept). Landing first paint ≈ 173 KB gz. Full detail in CLAUDE_ARCHIVE.md.
- **2026-06-28 session wrap:** shipped + Published verification UX (#60), F1 P3, F2, F3, F4 (all live); each typecheck+browser-verified; tree clean. **Known issue (minor, prod-safe):** raw-fetch handlers that `res.json()` on `res.ok` (e.g. F2 `loadCloseout`) throw "Unexpected token '<'" vs a *stale local backend* missing the `/api` route — never in prod; add a content-type check if it recurs.
- **PD test backlog:** Done+DEPLOYED: B1, B2, F1 P0-3, F2 (close-out), F3 (drawing revisions), F4 (H&S tab). Team Portal (#61) DEPLOYED 2026-07-03. Remaining (confirm scope with PD, per F2–F4 pattern): **F5** Daily Site Reports hub (company-wide aggregate of `daily_reports`), **F6** subbie/merchant docs at setup, **F7** Site Board live on-site count, **F8** Timeline programme link, **F9/F10** spikes.
- **2026-06-28 detail** (F4 H&S tab, F3 drawing revisions, F2 close-out, F1 P3, "Check your email" UX): all live; detail in CLAUDE_ARCHIVE.md.
- **2026-06-26 — Stripe billing hardening (DEPLOYED):** webhook ack-first+dedup (`stripe_webhook_events`), dup customer/sub guards + idempotency keys, beta-never-charged, `projects.ts` cap honours `betaAccess`. Detail in archive.
- **2026-06-26 — infra facts (reference):** **Deploy:** GitHub push ≠ deploy; prod = Replit **Publish** (PD clicks it — can't trigger from shell; builds from workspace snapshot → `Published your App` commit). Apex + `www` both → `34.111.179.208`. Workspace Stripe env is **LIVE** (`sk_live`); durable fix = deployment-scoped secrets (workspace=test). **TEST price IDs:** SOLO=`price_1TmaF0GXwfamd574D3Y5jVur`, TEAM=`price_1TmaF1GXwfamd574LzqikZYb`, PRO=`price_1TmaF1GXwfamd574IMeklGVt`. Local test: run `dist/index.mjs` with `sk_test`+test prices+`APP_URL=<dev-domain>` on a spare port. Full detail in CLAUDE_ARCHIVE.md.
