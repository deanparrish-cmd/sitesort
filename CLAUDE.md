...
- **2026-07-23 (3) — #87 pushed+Published+prod-verified; caught a gap in concurrent session's work (#88):** found a separate Replit Agent session building configurable per-document PIN sign-off in the same workspace — confirmed no conflict with #86, found+fixed a missing `ensure-schema.ts` entry that would have broken prod document endpoints. User Published (`df372dd`) — confirmed prod healthy for both #87 and #88. *(full detail in CLAUDE_ARCHIVE.md)*
- **2026-07-23 (2) — shipped #87 (activity deep-links + daily report portal sharing):** two explicit asks, both audited first (findings reported before implementing). Fixed 3 non-clickable-activity gaps via a new shared `lib/deep-link.ts` (caught a dead `?tab=photos` reference in the process); closed the one real Share-dialog gap by extending `daily_report` end-to-end through the portal-sharing pipeline, scoped to the site diary only per an explicit user decision. Hit and fixed a stale-running-server trap identical to #82's before trusting `check:layout`. `typecheck` clean, `check:layout` 96/96, full functional round-trip + browser screenshots. *(full detail in CLAUDE_ARCHIVE.md)*
- **2026-07-23 (1) — session opener + corrected stale #84/#85/#86 status + prod-verified all three:** routine housekeeping (trimmed CLAUDE.md, left the one-way GitHub sync alone), then found the prior session's "not yet committed" note was wrong — all three were already committed+Published under an unrelated checkpoint message (`898f8f0`) — corrected CLAUDE.md, then ran a full prod round-trip for all three against `www.sitesort.co.uk` (throwaway test fixtures, cleaned up after). Pushed to GitHub (`main → 9997d803`). *(full detail in CLAUDE_ARCHIVE.md)*
...
- **2026-07-20 (2)–(6) + surname audit:** Shipped **#77** (minimal-by-default Team Portal), **#78** (site issue archive/restore + admin hard-delete), **#79** (fixed a stale copy-on-write contact-name mirror — user-confirmed live), **#80** (fixed a stale-chunk-load crash on portal invite-accept via `lazyWithRetry`), plus a validation bugfix and a surname-data audit. All deployed via Replit Publish and pushed to GitHub (`main → 28468471`); #77/#79 prod-verified. *(full detail in CLAUDE_ARCHIVE.md)*
- **2026-07-19:** Shipped **#73** (Plant & Materials + issue closure reasons), **#74** (person-first contacts), **#75** (Daily Report in Team Portal), **#76** (Team Portal Messages). All committed, pushed, Published, prod-verified same day. *(full detail in CLAUDE_ARCHIVE.md)*
- **2026-07-17:** Responsive sweep #2 (`check:layout` Playwright audit, 176/176); #70/#71 alert-viewer + remove-people/archive/name-split; F6 contact documents #69; #68 Web Push closed out. All published+prod-verified. *(full detail in CLAUDE_ARCHIVE.md)*
