...
89. **Fix dead Overview-tab "Recent Activity" card** — `GET /api/projects/:id` hardcoded `recentActivity: []`; now builds a real feed (`getRecentActivity()` in `artifacts/api-server/src/routes/projects.ts`) merging recent document uploads, submitted photo/issue logs, daily site-update notes, and completed milestones, newest-first, capped at 8. Distinct from the manager-only `RecentActivityGlance`/`ProjectTeamActivity` portal-audit widgets, already working. **Verified** (2026-07-25): `typecheck` clean, confirmed live via API + browser screenshot.
...
91. **Document Allocate removed (folded into Share), project-level PM/approver authority added** — **(a)** Removed the "Allocate" document button: it duplicated Share→Team Portal→People but was actually worse — no `portal_shares` visibility rule, so a portal-only recipient got emailed but couldn't see/sign off in-portal. Folded Allocate's stronger tracked-email+notification into Share's audience resolution (`lib/document-distribution.ts`, shared with upload-time distribution too) — Share is now a strict superset, zero regression. **(b)** New `project_members.isProjectManager` grants PM-equivalent authority (triage issues, manage portal sharing, review member documents) scoped to ONE project without a company-wide role change. `lib/project-authority.ts#isProjectApprover()` — company admin/PM OR this flag, DB-fresh (instant revoke). Closed a real gap found along the way: issue triage and old Allocate had **no backend role gate at all** (UI-only), now enforced. Grant/revoke via a new "Project Manager" pill on the Team tab (company admin/PM-only) → `PATCH /projects/:id/members/:memberId/authority`. **Verified** (2026-07-25): `typecheck` clean, `check:layout` 96/96; full round-trip locally with a throwaway `site_worker` account — 403→granted→triage+close+allocate succeed, attribute correctly in `activity_log`, revoked→403 again; existing company PM unaffected. **DEPLOYED+prod-verified**: schema live, grant/revoke + UI toggle click confirmed, Allocate gone, Share→portal-shares confirmed creating both the distribution row AND the visibility rule together (the exact bug fixed) with a throwaway portal person who then saw the doc in-portal. Note: "log in AS the granted non-admin and triage" was only re-run locally (had DB access for a known password) — prod has no such access for a brand-new login, so that leg relies on local parity. Fixtures cleaned up. *(full detail in CLAUDE_ARCHIVE.md)*
...
- **2026-07-27 (2) — shipped #93** (Invoice Allocate + scoped Share, see #93). Found+fixed a real pre-existing gap: `PATCH /invoices/:id` and `POST /share-logs` had zero role gating for invoices — any authenticated company user could allocate/share via direct API calls. Published, prod-verified with a throwaway invoice fixture (negative-path 403 tested locally only, no prod DB access to make a second throwaway dashboard user). **Notes for next session:** the local api-server on `:8080` used by `check:layout`/manual verification is NOT auto-restarted on file changes — it's a plain `node dist/index.mjs` process a prior session leaves running; after backend changes, `pnpm --filter @workspace/api-server run build` then kill+restart that process before trusting live-server checks against it, or a stale build silently gives false negatives (lost ~15min to this mid-session). No known open issues from this session.
- **2026-07-27 (1) — shipped #92** (in-app User Guide across dashboard/portal/invite email, restyled with accent-orange callouts after "boring" feedback, see #92). Published, prod-verified.
- **2026-07-25 (3) — shipped #91** (Allocate/Share dedup + project-approver authority, see #91). Published, prod-verified.
- **2026-07-25 (2) — shipped #90**, three portal fixes (see #90). Published, prod-verified.
- **2026-07-25 (1) — shipped #89**, recentActivity gap (see above). Also: local `git pull` fails — `origin` is a synthetic API-pushed mirror, not shared history; use `push-robust.ts`.
- **2026-07-23 — corrected stale #84/85/86 status, shipped #87, fixed a gap in concurrent session's #88, Published.** *(full detail in CLAUDE_ARCHIVE.md)*
- **2026-07-21 and earlier:** see feature list above + CLAUDE_ARCHIVE.md (full session-by-session detail, including #73–#83, responsive sweep #2, F6, #68 Web Push).
