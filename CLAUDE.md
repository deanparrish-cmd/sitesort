...
- **2026-06-28 — F4: group MS/Permit/Safety under one H&S tab (DEPLOYED ✅ `9e6417e2`):** Frontend-only (`projects/detail.tsx`). Project "Compliance" tab relabelled **"H&S"** (value kept `permits` so `?tab=permits` deep links still work); heading → "Health & Safety". The lumped "Compliance Documents" list split into 3 by-type sections — **Method Statements (RAMS) / Permit Documents / Safety Documents** — each with count + per-section Upload (pre-selects type). Permits feature / Team Insurance / Share Log unchanged; Documents tab untouched (lightest option per PD). Verified typecheck + browser (label+heading+3 sections, 0 errors). ⏳ **TODO: push + PD Publish.**
- **2026-06-28 — F3: alphabetical drawing revisions (DEPLOYED ✅ `0078a6de`):** Drawings get an editable **revision label** (Rev A/B/C…) + revision-history view. `documents.revision` (ensure-schema). `documents.ts`: `versionToRevision()` (base-26); upload auto-assigns next letter for drawings (explicit wins; non-drawings null); PATCH accepts `revision`; tenant-scoped **`GET /documents/:id/revisions`** walks the supersede chain. `revision` added to Document + UploadDocumentRequest (codegen). `projects/detail.tsx`: `docRev()` shows "Rev X"; revision input on upload + edit; "Revisions" history dialog.
- **2026-06-28 — F2: project close-out / handover (DEPLOYED ✅ `2e4459d8`):** Manager-gated **"Close-out" tab**. New append-only **`project_closeouts`** table (ensure-schema). **`routes/closeout.ts`:** `GET /projects/:id/closeout` → 4 readiness checks (open snags, sub insurance, expired permits, pending sign-offs) + record; `POST` reuses the **doc sign-off PIN mechanism** (bcrypt + `pin-attempts` lockout), writes audit + `status=complete` in one txn; `POST .../reopen` → active (audit kept). Manager-only, tenant-scoped. Frontend: checklist + PIN dialog (incl. just-in-time PIN setup + handover note) + completed state w/ Re-open. Raw fetch (no codegen).
