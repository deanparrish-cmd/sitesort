...
- **Next-session notes (2026-07-11 wrap):** all work DEPLOYED+prod-verified, tree clean, nothing WIP. **Opportunity (not a bug):** point an uptime monitor at the new `GET /api/health` (200/503 + db status). **Backlog** (scope w/ PD first) — see PD test backlog below: F6–F10.
- **2026-07-11 — PROD 502-on-login FIX + hardening (DEPLOYED+prod-verified):** pg `Pool` had no `'error'` listener → dropped idle conn crashed the process → Replit restart loop → intermittent 502. Fixed w/ `pool.on('error')`, process-level handler guards, startup DB check, JSON 404 for `/api/*`, real `GET /api/health`. Full detail in CLAUDE_ARCHIVE.md.
