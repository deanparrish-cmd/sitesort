...
- **2026-07-03 session wrap — 3 changes, ALL DEPLOYED+Published+prod-verified:** (1) **Team Portal (#61)** — full invite-based per-project member portal + activity audit (see #61 for detail). STEP-0 investigate-first; PD chose "option 1" (dedicated `/api/portal` namespace). Backend E2E via curl (invite→accept→reads→containment-403s→activity→revoke); prod-verified `/api/portal/invite/x`→410 JSON (route live + `ensureSchema` ran on prod). (2) **Nav declutter** — removed Compliance Centre + Site Check-Ins from the sidebar (redundant with per-project tabs). (3) **Contacts invite removed** — dropped "Invite to SiteSort" from the Contacts page; inviting is now **project-only** via the Team Portal tab (adds member to project + portal on accept). Each: typecheck green → committed → pushed via push-robust → verified on GitHub main → PD Published → prod-verified in a real browser (login + assert). Tree clean, no WIP. **Loose ends (PD chose to leave):** dashboard still links "View Compliance Centre"/expiring alerts → now-unlinked `/compliance`; **dormant** subcontractor-invite backend (`/api/subcontractors/:id/invite` + `/register?invite=`, #36) is unreachable from UI but KEPT; compliance/checkins routes still deep-linkable. **NEXT = optional F5** (Daily Site Reports hub — confirm scope with PD first).
- **2026-06-29 — frontend performance pass (DEPLOYED ✅ `main → 16bc1bab`, Published):** code-splitting/lazy routes (`App.tsx` `React.lazy`+`Suspense`; 2.7 MB monolith → 88 chunks), `react-vendor` `manualChunks` (gotcha: DON'T name-chunk route-exclusive libs like recharts — drags shared helpers eager), images→WebP (PNG originals kept). Landing first paint ≈ 173 KB gz. Detail in archive.
...
- **2026-06-26 — Stripe billing hardening (DEPLOYED):** webhook ack-first+dedup, dup customer/sub guards + idempotency keys, beta-never-charged, `projects.ts` cap honours `betaAccess`. Detail in archive.
- **2026-06-26 — infra facts (reference):** **Deploy:** GitHub push ≠ deploy; prod = Replit **Publish** (PD clicks it — can't trigger from shell; builds from workspace snapshot). Apex + `www` → `34.111.179.208`. Workspace Stripe env is **LIVE** (`sk_live`); durable fix = deployment-scoped secrets. **Verify a deploy:** headless-browser login+assert (reuse `.claude/skills/browser-check` Nix Chromium; put script IN skill dir for ESM); rollout lags ~1-3 min. TEST price IDs + local-run in CLAUDE_ARCHIVE.md.
- **2026-06-24 & earlier** (F1 Phase 2 + all prior sessions): see CLAUDE_ARCHIVE.md.
