...
97. **Deep-link project alerts, expiry alerts, and overdue invoices to the specific record** — extends the `itemDeepLink` pattern (#64/#87) to three places that dead-ended on a general page: projects list "N Alerts" badge opens the exact pending document when every alert is on the same one (new `alertDocumentId` on `GET /api/projects`); dashboard expiry alerts carry `permitId`/`projectId`/`subcontractorId` so a permit alert opens that permit directly; invoices page Overdue card + dashboard's overdue-invoices tile open the single invoice directly when there's only one. **DEPLOYED+prod-verified** (2026-07-30, `main → 2fcca18a`): `typecheck` clean, `check:layout` 104/104, live headless-browser pass confirmed all paths against `www.sitesort.co.uk` (`alertDocumentId` present in the live API response, "1 Alerts" badge renders correctly, zero console/page errors).
...
- **2026-07-30 (2) — session-opener housekeeping, shipped #97:** CLAUDE.md was over the 30k trim threshold — moved features #94/#95/#96 full detail to CLAUDE_ARCHIVE.md. `git pull` correctly left alone — local/origin have diverged, expected for this repo's one-way sync model. The 8 locally-modified files turned out to be a complete, uncommitted deep-link feature (#97) from earlier in the session — rebuilt+restarted the local `:8080` server (was stale), `typecheck` clean, `check:layout` 104/104, live-verified all deep-link paths locally then committed+pushed (`main → 2fcca18a`) and re-verified against `www.sitesort.co.uk` after the user Published. **DEPLOYED+prod-verified.**
- **2026-07-30 (1) — shipped #94/#95/#96** (portal visual redesign; check-in insurance split-source + pinned-doc CAD-open fix; hamburger removed + nav-duplication cleanup). Published, prod-verified. **Standing gotcha:** local api-server on `:8080` is a plain `node dist/index.mjs` left running, NOT auto-restarted — rebuild + kill+restart after backend changes before trusting `check:layout`/manual checks.
