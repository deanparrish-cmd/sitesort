...
70. **Notification alert-viewer (Next/Previous)** — shared `<AlertViewer>` dialog replaces navigating away when clicking a notification. **DEPLOYED+prod-verified** (2026-07-17). *(full detail in CLAUDE_ARCHIVE.md)*
...
84. **Platform Admin restriction (`users.platformAdmin`)** — internal-staff flag distinct from company `role`; gates `/admin` server-side + nav; DB-managed allowlist replaces hardcoded emails. **DEPLOYED+prod-verified** (2026-07-23). *(full detail in CLAUDE_ARCHIVE.md)*
85. **Portal "Log a new item" for Plant & Materials** — members with `canUpdatePlantMaterials` can create new plant/materials entries directly from the portal, not just edit existing ones. **DEPLOYED+prod-verified** (2026-07-23). *(full detail in CLAUDE_ARCHIVE.md)*
86. **PIN-based document sign-off (Pending Sign-offs, dashboard + portal)** — every sign-off now requires the signer's hashed 4-digit PIN; new `pin_audit_log` table. **DEPLOYED+prod-verified** (2026-07-23). *(full detail in CLAUDE_ARCHIVE.md)*
87. **Activity-entry deep-links + daily reports join Team Portal sharing** — fixed 3 non-clickable-activity gaps via shared `lib/deep-link.ts`; closed the `daily_report`/`ShareModal` Team Portal gate gap. **DEPLOYED+prod-verified** (2026-07-23). *(full detail in CLAUDE_ARCHIVE.md)*
88. **Configurable per-document PIN sign-off** — built by a separate concurrent Replit Agent session in this workspace (noted here since it's now live): relaxes #86's universal PIN rule via `lib/signoff.ts#pinRequiredForDoc()` — safety-critical types always require PIN, others default to confirm-only unless a PM sets the new `requirePinSignoff` toggle. **Investigated and prod-verified by this session on request** (2026-07-23): no regression to #86, and found+fixed a missing `ensure-schema.ts` entry for the new column that would have 500'd every document endpoint in prod (`404e19b`). *(full detail in CLAUDE_ARCHIVE.md)*
89. **Fix dead Overview-tab "Recent Activity" card** — `GET /api/projects/:id` now builds a real feed (`getRecentActivity()`) instead of hardcoded `[]`. **Verified** (2026-07-25). *(full detail in CLAUDE_ARCHIVE.md)*
90. **Three portal fixes: nav/GET permission gating, explicit site manager, verified submit→view already worked.** **DEPLOYED+prod-verified** (2026-07-25). *(full detail in CLAUDE_ARCHIVE.md)*
91. **Document Allocate removed (folded into Share), project-level PM/approver authority added** — Allocate button removed (Share is now a strict superset via `lib/document-distribution.ts`); new `project_members.isProjectManager` grants scoped PM authority (`lib/project-authority.ts#isProjectApprover()`). **DEPLOYED+prod-verified** (2026-07-25). *(full detail in CLAUDE_ARCHIVE.md)*
92. **In-app User Guide (dashboard, portal, invite email) + colorful restyle** — single shared content source `lib/user-guide/src/index.ts`, rendered on 3 surfaces (dashboard `/user-guide`, portal `/portal/help`, public `/guide`), accent-orange restyle via `components/user-guide-view.tsx`. **DEPLOYED+prod-verified** (2026-07-27). *(full detail in CLAUDE_ARCHIVE.md)*
93. **Invoice Allocate + scoped Share (financial-data-safe)** — Invoices page gained a proper "Allocate"/"Re-allocate"/"Un-allocate" row action (reused/generalized the existing post-mark-paid project-picker dialog; un-allocating is now independent of paid/unpaid status, previously coupled). Share was already structurally excluded from Team Portal Everyone/Trade for invoices (`entityType="invoice"` was already outside `ShareModal`'s `PORTAL_ENTITY_TYPES` whitelist) — hardened further with a new `ShareModal` prop `secureViewUrl`: for invoices, Email/WhatsApp/a new "Copy secure link" button send an authenticated in-app deep link (`/invoices?invoice=<id>`) instead of the raw unauthenticated attachment URL, and the QR Code tab (which only ever exposed that raw URL) is hidden whenever `secureViewUrl` is set. **Closed a real backend gap**: `PATCH /invoices/:id` (projectId changes) and `POST /share-logs` (entityType=invoice) had **zero role gating** — any authenticated company user could allocate/share invoices via direct API calls; both now require company admin/PM or a per-project approver (`lib/project-authority.ts#isProjectApprover()`, reused verbatim from #91) on the relevant project. Frontend hides Allocate/Share for non-managers on the company-wide page; the project Finances tab's Share button now gates on `isProjectApprover` instead of being ungated. Zero schema changes needed. **DEPLOYED+prod-verified** (2026-07-27): `typecheck` clean, `check:layout` 102/102; full round-trip against prod with a throwaway invoice (create→allocate→confirms under project Finances-tab data→share by email→confirms logged with correct attribution→deleted); negative-path 403 for a throwaway `site_worker` verified locally (no DB access on prod to make a second throwaway user — relies on local parity + code identity with #91's already-prod-verified `isProjectApprover`).

94. **Team Portal visual redesign — big/colourful for on-site workers** — portal-only (no dashboard/data/permission change): new shared `components/portal-ui.tsx` (`PortalTile`/`PortalButton`/`StatusPill`, wayfinding `TileTheme` palette) + a solid-fill status-colour system in `section.tsx` (`GOOD`/`WARNING`/`BAD` = `bg-success`/`bg-warning`/`bg-destructive`, not pastel outlines). Tap targets ≥56px, base type bumped, tactile `active:scale` feedback everywhere. Home gained a big coloured quick-access tile grid (reuses `SECTION_NAV`'s own permission filter) above the unchanged 5-box structure; documents gained a real green "CURRENT" badge alongside the existing red "SUPERSEDED". **Verified**: `typecheck` clean, `check:layout` 102/102; live-verified at 360px in simulated standalone/PWA mode via a throwaway portal-member fixture (cleaned up after) — zero console/page errors. Not yet Published — flagged to the user.

95. **Fix check-in insurance gate vs contact card split-source, and pinned site-board docs not opening** — two bugs, both split-source. **(a)** The Contacts directory's "Insurance OK" badge (`combinedInsuranceStatus` in `subcontractors.ts`) counts company-level `insurance_records` PLUS any filed insurance-named `person_certifications` row, but the site-board check-in gate (`qr.ts`) only ever queried `insurance_records` — a contact insured solely via a person-level cert (e.g. a self-employed contact, common post-#74) passed the card but was wrongly turned away at check-in. Extracted the shared logic into new `lib/insurance.ts` (`subcontractorInsuranceStatus()`), used by both; check-in now blocks only on `expired`/`none`, passes `valid`/`expiring_soon` — same not-yet-expired-passes rule as everywhere else. **(b)** Pinned site-board documents (both the public scanned page and the portal) opened via a raw `window.open(fileUrl)`, unlike every other document view in the app (`openDocument()`/`openDocFile()`), which is CAD-aware (DWG/DXF/etc. can't render in a browser tab and must download instead) — a pinned CAD drawing silently failed to open (blank tab), while regular "Shared with me" documents worked fine. Fixed both `site-board.tsx` and portal `section.tsx`'s pinned-doc button to use the same CAD-aware helper. **Verified**: `typecheck` clean, `check:layout` 102/102; full local round-trip — a real self-employed contact with ONLY a person-level insurance cert (matching the reported Amy Parrish scenario) now passes check-in (201), the same contact with an expired-only cert still correctly blocks (403), and an expiring-soon-but-not-expired cert still passes (201); pinned-doc open confirmed end-to-end via a real headless-browser click on a genuinely uploaded file. All test fixtures cleaned up.
...
- **2026-07-30 — shipped #94/#95** (portal visual redesign; check-in insurance split-source + pinned-doc CAD-open fix). Published, prod-verified. **Standing gotcha:** local api-server on `:8080` is a plain `node dist/index.mjs` left running, NOT auto-restarted — rebuild + kill+restart after backend changes before trusting `check:layout`/manual checks.
- **2026-07-27 — shipped #92/#93** (in-app User Guide; Invoice Allocate + scoped Share, closed zero role-gating on `PATCH /invoices/:id`/`POST /share-logs`). Published, prod-verified.
- **2026-07-25 — shipped #89/#90/#91** (recentActivity gap, 3 portal fixes, Allocate/Share dedup + project-approver authority). All published, prod-verified. Local `git pull` fails — `origin` is a synthetic API-pushed mirror, not shared history; use `push-robust.ts`.
- **2026-07-23 and earlier:** see feature list above + CLAUDE_ARCHIVE.md.
