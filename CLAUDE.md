...
- **NEXT SESSION TODO — PD test backlog (resume here):** Working PD's prioritised backlog. **Done + DEPLOYED+live:** B1, B2, F1 Phase 0+1, F1 Phase 2 (permits). **Done, NOT yet deployed:** F1 Phase 3 (insurance certs — see 2026-06-28 entry). **NEXT = F2** close-out+sign-off. Then remaining: **F3** alphabetical drawing revisions, **F4** group MS/Permit/Safety under one H&S tab, **F5** Daily Site Reports hub, **F6** subbie/merchant docs at setup, **F7** Site Board live on-site count, **F8** Timeline programme link (decision-first), **F9/F10** spikes only. (Optional F1 P3 follow-ups: surface assignee/OVERDUE on the Compliance Centre insurance rows too, and add Assign-to/Due-by to the insurance upload flow on the compliance page.)
- **2026-06-28 — F1 Phase 3: insurance cert accountability (committed `0912609`, NOT yet pushed/deployed):** Mirrors Phase 2 for subcontractor insurance. **Schema:** `insurance_records` + `assigned_to_user_id` (FK users) + `due_date`, both idempotent in `ensure-schema.ts`. **Backend:** `serializeInsuranceRecords` (subcontractors.ts) adds `assignedToUserId`/`assignedToUserName`/`dueDate` + derived `overdue = isOverdue(dueDate, !!archivedAt)`; status now via shared `expiry.ts` (`active`→`valid`) in subcontractors.ts **and** team.ts. New tenant-scoped, IDOR-safe **`PATCH /subcontractors/:id/insurance/:recordId`** (edit/reassign); POST insurance accepts `assignedToUserId`/`dueDate`. **Spec+codegen:** `InsuranceRecord`/`AddInsuranceRequest` extended, `UpdateInsuranceRequest` + `updateInsuranceRecord` added; api-zod + api-client-react regenerated. **Frontend** (contacts directory `subcontractors/index.tsx`): insurance rows show assignee + due-by + **OVERDUE** badge; manager-gated "Insurance Accountability" dialog (assignee dropdown from `/api/users` + due-date) → PATCH → reload. Verified: ✅ full typecheck; API PATCH assign/reassign/clear + overdue + cross-tenant 404; real browser single-origin `:8099` (6/6 checks, 0 errors, card + dialog screenshots); demo record reverted. ⏳ **TODO: push to GitHub + PD Publish.** (compliance.ts/qr.ts insurance gates already correct — left as-is; see optional follow-ups in NEXT TODO.)
- **2026-06-28 — "Check your email" screen UX (Feature #60 follow-up; DEPLOYED ✅ — verified live `index-pezeZJV1.js` on sitesort.co.uk):** post-signup verification screen (`register.tsx`): (1) spam line "check your spam or junk folder — and mark it 'not spam'…"; (2) **rate-limited resend** — 45s frontend countdown ("Resend available in Ns", disabled) + silent **30s per-email in-memory throttle** in `/auth/resend-verification` (`resendThrottled()`, still returns `{success:true}` for anti-enumeration); (3) **"Wrong email? Go back and edit"** returns to the form with the typo preserved (`editEmail()`). Verified typecheck + browser (9/9) + DB token-rotation. Pushed `main → a98f5b26`, Published by PD (`42459ba`).
- **2026-06-27 — Real email verification on registration (Feature #60; see feature ledger #60):** register creates `emailVerified:false` + token/24h-expiry, sends verification (not welcome) email, issues NO JWT → `{requiresVerification, email}`; welcome fires only post-verify; `ensure-schema.ts` adds the `email_verification*` cols; `register.tsx` shows "Check your email". Verified end-to-end + browser. Pushed `main → 296490d6`. ⏳ TODO: PD Publish.
