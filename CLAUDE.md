...
- **2026-06-26 — Stripe billing bug-fix (3 fixes; NOT yet deployed):** All in `artifacts/api-server/src/routes/billing.ts` + one frontend branch in `pages/auth/register.tsx`. **(1) Webhook timeout + idempotency** — `/billing/webhook` verifies the signature, responds `200` *immediately*, then runs the event in fire-and-forget `processWebhookEvent` (own try/catch, logs only, no `res`) — fixes the ~10s delivery timeout → failed-then-retried deliveries. **Event-ID dedup ledger:** new `stripe_webhook_events` table (PK = Stripe `evt_…` id; drizzle schema + index export + **ensure-schema** boot migration since prod DB is separate). `claimEvent()` atomically `INSERT…ON CONFLICT DO NOTHING…RETURNING` (mirrors `claimMilestone`); `processWebhookEvent` skips + logs "duplicate" if claim returns false (before any side effect); `releaseEvent()` on handler throw so a redelivery can re-process; claim-insert error → process anyway (handlers upsert-idempotent). Stops retried/replayed events double-processing. **(2) Duplicate subscriptions** — `/billing/checkout` now resolves an existing customer (stored `company.stripeCustomerId` → else `stripe.customers.list({email})`), **persists** it, passes `customer` not `customer_email`, and if a customer already has an `active`/`trialing` sub returns `{alreadySubscribed:true}` without creating a second. **(3) Beta users billed** — chose **skip-Stripe** over a 100%-off coupon (no customer/sub/£0-invoice footprint): if `company.betaAccess`, checkout sets `subscriptionStatus:"active"` + tier and returns `{beta:true}` without touching Stripe. **Frontend** (`register.tsx`): submit handler routes `{beta}`/`{alreadySubscribed}` → `/dashboard` instead of erroring on missing `url`. **Verified:** full typecheck ✅; browser smoke test (Vite `:18299`) — `/register` renders, plan-select → Create Account form, no console/page errors. **NOT verified:** the Stripe-touching API paths were deliberately NOT driven — this env holds the **LIVE** Stripe key + live price IDs, so end-to-end belongs in a **Stripe test-mode** pass (beta bypass / idempotency repeat-signup / `stripe trigger` webhook timing). Not committed, not pushed, not deployed yet.
