...
- **2026-06-28 — F4: group MS/Permit/Safety under one H&S tab (DEPLOYED ✅ `9e6417e2`):** Frontend-only (`projects/detail.tsx`). Project "Compliance" tab relabelled **"H&S"** (value kept `permits` so `?tab=permits` deep links still work); heading → "Health & Safety". The lumped "Compliance Documents" list split into 3 by-type sections — **Method Statements (RAMS) / Permit Documents / Safety Documents** — each with count + per-section Upload. Permits/Team Insurance/Share Log unchanged; Documents tab untouched. Verified typecheck + browser (label+heading+3 sections, 0 errors). ⏳ **TODO: push + PD Publish.**
