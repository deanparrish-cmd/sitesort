...
- **2026-07-27 (2) — shipped #93** (Invoice Allocate + scoped Share). Found+fixed zero role gating on `PATCH /invoices/:id`/`POST /share-logs`. Published, prod-verified. **Standing gotcha:** local api-server on `:8080` is a plain `node dist/index.mjs` left running, NOT auto-restarted — after backend changes, rebuild + kill+restart before trusting `check:layout`/manual checks against it (lost ~15min to this once).
- **2026-07-27 (1) — shipped #92** (in-app User Guide, restyled after "boring" feedback). Published, prod-verified.
...
- **2026-07-23 — corrected stale #84/85/86 status, shipped #87, fixed a gap in concurrent session's #88, Published.**
- **2026-07-21 and earlier:** see feature list above + CLAUDE_ARCHIVE.md.
