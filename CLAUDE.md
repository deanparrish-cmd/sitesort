...
- **2026-06-29 session wrap:** Did one out-of-band task — the frontend performance pass (entry below): code-splitting/lazy routes, WebP images, vendor chunking. **DEPLOYED+Published** (`main → 16bc1bab`; docs `18e0483a`). typecheck+build clean, browser-verified across desktop/tablet/mobile, 0 page/console errors. **No new issues discovered** (the only known minor item remains the prod-safe stale-local-backend `loadCloseout` `res.json()` HTML-fallback, see below). Tree clean, no WIP. **NEXT unchanged = F5** (Daily Site Reports hub — confirm scope with PD first).
- **2026-06-29 — frontend performance pass (DEPLOYED ✅, pushed `main → 16bc1bab`, PD Published):** (1) **Code splitting / lazy routes** — `App.tsx` now `React.lazy` per page + `<Suspense>` full-screen `PageLoader` spinner (NotFound kept eager). Killed the **single 2.7 MB JS monolith** → 88 chunks. (2) **`vite.config.ts` `manualChunks`** — only a `react-vendor` chunk (react/react-dom/scheduler/wouter/@tanstack, all eager anyway). **Gotcha learned:** force-grouping recharts into a named `charts` chunk drags a shared helper into the eager graph and makes the entry HTML modulepreload ~107 KB of recharts on the landing page — so DON'T name-chunk route-exclusive libs; let Rollup auto-split them (recharts now loads only on `/admin`). (3) **Images → WebP** (ImageMagick `magick`): construction-hero 1.3 MB→44 KB, auth-bg 1.6 MB→116 KB, logo 104 KB→20 KB, built_for_beam→12 KB; refs updated in landing/login/register/sidebar-layout, hero got `fetchPriority="high"`. PNG originals KEPT (non-destructive; >1 MB ones are skipped by the push proxy anyway). (4) **Loading spinners** — audited; every data-fetching page already had skeleton/spinner loaders, so no churn. **Result:** landing first paint ≈ **173 KB gzipped** (was ~700 KB gz JS + 1.3 MB PNG). Verified in real browser (landing + login × desktop 1440 / tablet 768 / mobile 390): all render, 0 page/console errors, WebP 200s.
- **2026-06-28 session wrap:** shipped + Published verification UX (#60), F1 P3, F2, F3, F4 (all live); each typecheck+browser-verified, demo data reverted; tree clean, no WIP. **Known issue (minor, prod-safe):** raw-fetch handlers that `res.json()` on `res.ok` (e.g. F2 `loadCloseout`) throw "Unexpected token '<'" against a *stale local backend* missing the `/api` route (SPA `index.html` served as 200 fallback) — never in prod; harden with a content-type check if it recurs.
- **NEXT SESSION TODO — PD test backlog (resume here):** Working PD's prioritised backlog. **Done + DEPLOYED+live:** B1, B2, F1 Phase 0+1+2+3, F2 (close-out), F3 (drawing revisions), F4 (H&S tab — `9e6417e2`). **NEXT = F5** Daily Site Reports hub (likely a company-wide hub aggregating `daily_reports` across projects, like the Site Check-Ins/Issues pages — there's already a per-project "Daily Reports" tab + `daily_reports` table; **confirm scope with PD first**, per the F2–F4 pattern). Then: **F6** subbie/merchant docs at setup, **F7** Site Board live on-site count, **F8** Timeline programme link (decision-first), **F9/F10** spikes only. (Optional follow-ups: F1 P3 — assignee/OVERDUE on Compliance Centre rows; F2 — surface close-out on Compliance Centre/dashboard.)
- **2026-06-28 detail (F4 H&S tab, F3 drawing revisions, F2 close-out, F1 P3 insurance accountability, "Check your email" UX):** all DEPLOYED+live; full detail archived to CLAUDE_ARCHIVE.md (2026-06-28 session detail).
- **2026-06-26 — Stripe billing hardening (DEPLOYED; detail in archive):** webhook ack-first + dedup (`stripe_webhook_events`), dup customer/sub guards + idempotency keys, beta-never-charged. Beta limits resolved (`projects.ts` cap honours `betaAccess`). `dbed53bd`/`6d8f434`.
...
- **2026-06-24 and earlier** (F1 Phase 2 permits/expiry, all 2026-06-23 and prior sessions): see CLAUDE_ARCHIVE.md.
