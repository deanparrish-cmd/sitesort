...
89. **Fix dead Overview-tab "Recent Activity" card** — `GET /api/projects/:id` hardcoded `recentActivity: []`; the route now builds a real feed (`getRecentActivity()` in `artifacts/api-server/src/routes/projects.ts`) merging recent document uploads, submitted photo/issue logs, daily site-update notes, and completed milestones, sorted newest-first, capped at 8. Distinct from the manager-only `RecentActivityGlance`/`ProjectTeamActivity` portal-audit widgets, which were already working. **Verified** (2026-07-25): `typecheck` clean, confirmed live via API + browser screenshot (real entries render, no console errors). Not yet Published.
...
- **2026-07-25 — fixed the recentActivity gap flagged at the end of the 2026-07-23 session:** shipped **#89** — `GET /api/projects/:id` no longer hardcodes `recentActivity: []`; it now aggregates real document/photo/note/milestone events per project. `typecheck` clean, verified via direct API call and a logged-in browser screenshot of the Overview tab (real entries, no console errors). Committed, not yet Published to prod. Also noted: local `git pull` fails — `main` and `origin/main` have diverged (92 vs 65 commits), consistent with `origin` being a synthetic API-pushed mirror rather than real shared history; left unresolved pending user decision, use `push-robust.ts` to push.
...
- **2026-07-21 (2) — shipped #82, follow-up to #81:** whole-login revoke via the "Portal member" pill, badge/pill reordering, single invite-creation path confirmed. `f6ec798`/`bb91e29`. **DEPLOYED+prod-verified**, but had a parity gap fixed same-day by **#83**. *(full detail in CLAUDE_ARCHIVE.md)*
...
- **2026-07-20 and earlier:** see feature list above + CLAUDE_ARCHIVE.md (full session-by-session detail, including #73–#81, responsive sweep #2, F6, #68 Web Push).
