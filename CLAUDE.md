...
81. **Inline portal-permission toggles on Team tab cards** — the Site Issues/Plant & Materials/Daily Report grants from #77 existed and worked (`requirePortalPermission`, `PATCH .../members/:id/permissions`) but were buried behind a click-to-open dropdown, which the user flagged as not visible across multiple sessions. Presentational-only fix: the same 3 `togglePermission` checkboxes now render as always-visible pills directly on the "Portal member" card (`PermissionTogglePill` in `portal-people.tsx`), matching how Invite/Notes/Docs/Share/Remove already render inline; only "Revoke access" stayed in a small "⋯" menu. No permission-model changes. **DEPLOYED+prod-verified** (2026-07-21): `typecheck` clean, `check:layout` 96/96, and the grant→200/revoke→403 `GET /api/portal/site-issues` round-trip plus inline-rendering screenshot confirmed both locally and directly against `www.sitesort.co.uk`.
...
83. **Portal permission card row order + pre-accept permission parity + full functional verification** — reordered the card to badge-then-pill (row 1) / 3 toggles together (row 2) / Notes-Docs-Share-Remove (row 3). Root-caused the real parity bug behind #82's still-inconsistent toggles: a pending invitee is already a project team member (that's how their card exists to invite from), so their `project_members` row already exists — `portalStatusFor` just wasn't surfacing its permission columns for "invited" status. Fixed, plus two edge cases it surfaced: the accept endpoint matched only the brand-new `userId` (would've silently duplicated the row and dropped pre-set permissions — now also matches `personId` and updates in place), and the pending-invite revoke handler nulled `personId` on a `userId`-null row, orphaning it (now resets permissions, keeps the row). **DEPLOYED+prod-verified** (2026-07-21): full a→f functional chain (grant persists/survives reload → member's nav gains the section → member views → member creates/edits and it saves → reaches the PM with correct attribution → untick 403s the write endpoint) passed 18/18 for Site Issues, Plant & Materials, and Daily Reports, against `www.sitesort.co.uk` directly. *(full detail in CLAUDE_ARCHIVE.md)*
84. **Platform Admin restriction (`users.platformAdmin`)** — SiteSort's own internal-staff flag, distinct from `role` (a customer's admin/pm/worker role within their own company); gates the whole `/admin` section server-side (`requireAdmin` re-checks the DB on every request, not the JWT) and in the sidebar nav. Seeded Dean/Amy once; an Admin-section "Platform Admins" table (search + toggle) now manages the list going forward — no more hardcoded email allowlist. Self-revoke blocked. **Built+verified locally** (2026-07-21): `typecheck` clean, `check:layout` 96/96.
85. **Portal "Log a new item" for Plant & Materials** — members with `canUpdatePlantMaterials` can create a brand-new plant/equipment or materials entry directly from the portal (`POST /api/portal/plant-materials`), not just edit existing items; creates live immediately (no draft stage) and notifies the project's managers. **Built+verified locally** (2026-07-21): `typecheck` clean, `check:layout` 96/96.
86. **PIN-based document sign-off (Pending Sign-offs, dashboard + portal)** — every document sign-off (not just drawings/method statements/safety as before) now requires the signer's own hashed 4-digit PIN, making it an attributable signature: viewing a doc never needs a PIN and is logged separately from signing it off. New portal-side twins of the existing dashboard flow: `POST /portal/documents/:id/view`, `POST /portal/documents/:id/acknowledge` (both gated on `portal_shares` visibility, safety docs always open), `POST /portal/pin` (set/reset, password-reverified — doubles as "forgot PIN"). New `pin_audit_log` table logs every PIN set/reset (who, when, set vs reset — never the PIN itself); shared `lib/pin.ts#setUserPin` backs both `/auth/pin` and `/portal/pin`. Dashboard Compliance page's "Pending Sign-offs" now shows a **named** per-recipient breakdown (not just a count) with a "Sign off" action when the viewer themself is a pending recipient; new `hooks/use-sign-off-flow.ts` is the shared PIN-entry state machine behind both the dashboard's dialog and the portal's inline (mobile-first) card. Signing off a new revision (a supersede) is a separate sign-off, since each revision is its own document row with fresh distributions. **Built+verified locally** (2026-07-21): `typecheck` clean, `check:layout` 96/96 at 360px+768px, plus a full functional round-trip against a running local server — correct PIN records `{userId, userName, userRole, documentId, documentVersion, action:"acknowledged", signedOffWithPin:true, ipAddress, userAgent, createdAt}` in `acknowledgment_audit_log` and clears the doc from Pending Sign-offs; wrong PIN rejected without recording anything and locks out after 5 attempts (429, 15 min, Redis-backed); superseding a signed-off doc re-added it to Pending Sign-offs as a fresh, separate sign-off; PIN confirmed stored as a bcrypt hash (`$2b$10$…`) in `users.pin_hash`, never returned by any endpoint (`/auth/me` only exposes a `hasPin` boolean) and not selected by the admin user-list endpoint. Same chain re-verified against a real portal-member login (`/portal/*` endpoints). Not yet committed.
...
- **2026-07-21 (4) — resumed an in-progress session (#84/#85 already coded but uncommitted) and shipped #86:** picked up mid-flight work left uncommitted from earlier in the day (**#84** Platform Admin restriction, **#85** portal "Log a new item" for Plant & Materials) — verified both (`typecheck`, `check:layout` 96/96) rather than assuming they were done, since neither had a CLAUDE.md entry yet. Then built **#86**, a large spec: PIN-based sign-off for the whole Pending Sign-offs flow, dashboard AND portal. Investigated first — most of the backend (hashed PIN storage, rate-limited PIN verification, append-only `acknowledgment_audit_log`, the dashboard sign-off dialog) already existed from an earlier feature, but PIN was only enforced for 3 doc types, the portal had NO sign-off endpoint at all (view-tracking only), and the Compliance page's "Pending Sign-offs" showed a bare count with no way to actually sign off or see who was pending. Made PIN universal for every sign-off (dropped the type gate), built the portal's missing view/acknowledge/pin endpoints as twins of the dashboard's, added a named per-recipient breakdown to Compliance, and extracted a shared `useSignOffFlow` state-machine hook so the dashboard's dialog and the portal's inline mobile-first card don't duplicate the PIN logic. New `pin_audit_log` table logs every set/reset. Verified with a full functional round-trip on a running local server (not just render checks) for both a dashboard user and a real portal-member login: correct PIN signs off and records the full audit shape, wrong PIN is rejected and locks out after 5 attempts, superseding a signed-off doc correctly re-opens a fresh separate sign-off, and the PIN is confirmed stored only as a bcrypt hash and never returned by any endpoint. `typecheck` clean, `check:layout` 96/96 at 360px+768px. Test fixtures (a throwaway portal user + 4 test documents) created and cleaned up directly against the local DB. **Not yet committed or deployed** — awaiting the user's go-ahead.
- **2026-07-21 (3) — shipped #83, same-day root-cause fix on top of #82:** user reported #82's card was still wrong in practice — badge/pill order needed reversing (badge first), the 3 toggles needed to be one grouped row, and — the important one — email-invited vs link-invited people still showed inconsistent/no toggles while pending. Root-caused rather than patching symptoms: a pending invitee is ALWAYS already a project team member (that's how their card exists to invite from), so their `project_members` row already existed — `portalStatusFor` just wasn't copying its permission columns onto the "invited" status object. Fixed that plus two real edge cases it surfaced (accept-endpoint row-matching would've silently duplicated the row and dropped pre-set permissions; the pending-invite revoke handler would've orphaned a `userId`-null row). Then ran a genuine end-to-end functional test (not render checks) for all 3 sections — full a→f chain, 18/18 passed both locally and against `www.sitesort.co.uk` directly, with screenshots of the live card and the live portal nav gaining/losing sections. Committed `577c796`. User Published (`3e79327`). **DEPLOYED+prod-verified.**
- **2026-07-21 (2) — shipped #82, a same-day follow-up to #81:** user reported 3 concrete gaps in #81's card: status pill needed to sit above the role/type tag, the "Portal member" pill itself should BE the whole-login on/off (with confirm, distinguished from the per-section toggles), and share-link vs email invite needed the same permission guarantees. Investigated first rather than assuming — found the whole-access revoke endpoint (`POST /invites/:inviteId/revoke`) already did the right thing (kills sessions, clears membership), and that there's only one invite-creation endpoint, never carrying permission fields — so most of the "fix" was wiring + confirm dialogs, not new backend logic. While verifying, caught that the local `:8080` test server was serving a stale pre-edit build (the `api-server dev` process is a one-shot build+start, not a watcher) — rebuilt + restarted it before trusting `check:layout`'s result. `typecheck` clean, `check:layout` 96/96 against the fresh build. Committed `f6ec798`. User Published (`bb91e29`). **DEPLOYED+prod-verified**, but turned out to still have a real parity gap — see **#83** above, shipped the same day.
- **2026-07-21 — shipped #81, the priority carried over from the previous wrap-up:** moved the 3 portal-permission checkboxes (Site Issues/Plant & Materials/Daily Report) out of the "Portal member" pill's dropdown and onto the card as always-visible pills — presentational move only, no permission-model changes, per the explicit constraint left in the prior session's note. Verified locally with `typecheck`, `check:layout` (96/96), and a full grant→200/revoke→403 API round-trip against a disposable test person (cleaned up after) plus a browser screenshot confirming inline rendering. User Published via Replit UI (checkpoint commit `04045dd` swept in the workspace changes, the established pattern). **DEPLOYED+prod-verified**: repeated the same grant→200/revoke→403 round-trip directly against `www.sitesort.co.uk` with a throwaway `@example.com` test person (archived after cleanup), plus a real-browser screenshot of the live Team tab confirming the pills render inline on the deployed bundle.
