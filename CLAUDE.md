...
- `artifacts/sitesort/src/components/layout/sidebar-layout.tsx` — `VOICE_COMMANDS` map + regex matches in `matchVoiceCommand` for subcontractor find (with inline term), compliance upload, and compliance find/recall (with inline term); all action commands encode params into URLs
- `artifacts/sitesort/src/pages/projects/index.tsx` — `useEffect` reads `?new=1`, opens modal or redirects to billing
- `artifacts/sitesort/src/pages/invoices/index.tsx` — `toggleVoiceSearch` → `useCallback`; `useEffect` reads `?new=1` / `?recall=1`
- `artifacts/sitesort/src/pages/subcontractors/index.tsx` — `toggleVoiceSearch` → `useCallback`; `useEffect` reads `?new=1` / `?q=<term>` / `?find=1`
- `artifacts/sitesort/src/pages/compliance/index.tsx` — `toggleVoice` → `useCallback`; `highlightUpload` state pulses upload banner on `?upload=1`; `useEffect` reads `?upload=1` / `?q=<term>` / `?find=1`
- `artifacts/sitesort/src/pages/messages/index.tsx` — `Mic`/`MicOff` icons; `pendingTo`/`autoDictate`/`dictating` state; `dictateRef`; `startDictation`/`stopDictation`/`toggleDictation` callbacks; param detection `useEffect` for `?new=1` / `?to=<name>` / `?dictate=1`; name-matching effect against `teamUsers`; auto-start dictation effect when `autoDictate` + `activeConv` are both set; mic button in compose bar appends spoken text to draft
- `artifacts/sitesort/src/pages/projects/index.tsx` — safety issue modal: project picker (auto-selects if one active project), voice-dictated description textarea with mic button, zone field, optional photo upload; submits to `POST /api/projects/:id/photos` with `category: safety_concern`; shows reference number on success; managers notified via existing notification + email flow
- `artifacts/api-server/src/routes/photos.ts` — `photoUrl` made optional in request validation; stored as `null` when not provided
- `lib/db/src/schema/photos.ts` — `photoUrl` column made nullable; schema pushed to DB
