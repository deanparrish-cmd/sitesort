...
- **NEXT SESSION TODO — PD test backlog (resume here):** Working PD's prioritised backlog. **Done + DEPLOYED+live:** B1, B2, F1 Phase 0+1, F1 Phase 2 (permits — see 2026-06-24 entry). **NEXT = F1 Phase 3 (insurance certs)**, mirroring Phase 2: `insurance_records` gets new `assignedToUserId`+`dueDate` cols (no existing responsible field, so add both) via `ensure-schema.ts`; serialize `dueDate`+`overdue` (`isOverdue(dueDate, !!archivedAt)`); reuse the shared `expiry.ts` helpers (already built) for insurance status in `subcontractors.ts`/`team.ts`/`compliance.ts`/`qr.ts` insurance gate; add Assign-to+Due-by + OVERDUE badge + edit/reassign on the contact insurance UI. Then remaining: **F2** close-out+sign-off, **F3** alphabetical drawing revisions, **F4** group MS/Permit/Safety under one H&S tab, **F5** Daily Site Reports hub, **F6** subbie/merchant docs at setup, **F7** Site Board live on-site count, **F8** Timeline programme link (decision-first), **F9/F10** spikes only.
- **2026-06-26 — Stripe billing hardening (ALL DEPLOYED + live on www):** Four fixes across `billing.ts`, `admin.ts`, `register.tsx`. **(W) Webhook ack-first + dedup** — `/billing/webhook` verifies sig, responds `200` *immediately*, then fire-and-forget `processWebhookEvent` (fixes ~10s timeout→retries). New `stripe_webhook_events` ledger (PK=`evt_…`; drizzle + index + **ensure-schema** boot migration). `claimEvent()` atomic `INSERT…ON CONFLICT DO NOTHING…RETURNING`; duplicate events skipped before side effects; `releaseEvent()` on throw. (commit `053ccb5`/`2eb7de1`) **(1) Dup customers/subs** — `/billing/checkout` reuses customer (stored `stripeCustomerId`→else `customers.list({email})`, persists), and if an `active`/`trialing` sub exists returns `{alreadySubscribed:true}`. **Idempotency keys:** explicit `customers.create` keyed `cust:<companyId>` + `checkout.sessions.create` keyed `checkout:<userId>:<plan>` (double-click→one customer+sub). Frontend `register.tsx`: `useRef` re-entrancy guard; `alreadySubscribed`→toast + `/settings?tab=billing`. (commit `d02985f`/`2e27d6f`) **(2) Beta never charged — skip-Stripe** — `admin.ts` PATCH `/admin/companies/:id/beta-access`: GRANT sets `betaAccess+status=active+tier="pro"` FIRST (so the cancellation webhook sees beta & skips — no downgrade race), THEN cancels live Stripe sub (try/catch → `warning` on failure); REVOKE sets `status="incomplete"+tier="free"` → CheckoutGate → normal checkout. `tier="pro"` because backend project/sub/doc limits are **tier-based and ignore `betaAccess`** (`projects.ts:25` etc.). `billing.ts` `isCompanyBeta()` guard skips both webhook sub-handlers for beta cos. (commit `458749e`/`0488f49`) **All verified in Stripe TEST mode** (programmatic: dup-guard returns alreadySubscribed w/ 1 customer+1 sub; beta grant cancels sub→pro/active, fake `subscription.deleted` webhook ignored, revoke→incomplete) then deployed + live-confirmed via publish-lineage (each `Published your App` is a child of its fix commit). Pushed GitHub `main → dbed53bd`. **Beta limits RESOLVED (`6d8f434`):** the project cap is the ONLY tier-based limit (`projects.ts`; subcontractors/documents have none) — it now honours `betaAccess` directly (beta→Infinity); dropped the `tier="pro"` masquerade so beta cos keep their real tier. Verified: free co capped at 1 (2nd→403), beta co creates 2+3.
- **2026-06-26 — infra facts (reference):** **DNS:** apex `sitesort.co.uk` → `192.64.119.235` (registrar **parking IP — hangs/times out**); the app is **`www.sitesort.co.uk`** → `34.111.179.208` (Replit autoscale). Stripe's live webhook endpoint (`we_1Tfci…`) already → www, so this never broke Stripe; but the apex should be repointed (Replit custom domain + registrar A record) or it stays dead. **Deploy:** GitHub push ≠ deploy; prod = Replit **Publish** (autoscale, builds api-server+frontend from workspace snapshot; creates `Published your App` commit). **Cannot be triggered from the shell** — PD clicks Publish. **Stripe keys:** workspace env currently **LIVE** (`sk_live`); `STRIPE_PRICE_*`+`APP_URL` in `.replit [userenv.shared]`. Durable fix = Replit **deployment-scoped secrets** (workspace=test, deployment=live) so dev testing is always safe. **TEST-mode price IDs (already created in Stripe test):** SOLO=`price_1TmaF0GXwfamd574D3Y5jVur`, TEAM=`price_1TmaF1GXwfamd574LzqikZYb`, PRO=`price_1TmaF1GXwfamd574IMeklGVt`. To test locally: run `dist/index.mjs` with `STRIPE_SECRET_KEY=sk_test…`+test prices+`APP_URL=<dev-domain>` on a spare port; card `4242 4242 4242 4242`.
- **2026-06-24 — F1 Phase 2 (permits accountability + expiry consolidation):** DEPLOYED + live + pushed (`main → 998c2bad`). Added `permits.due_date`, shared **`expiry.ts`** helper (canonical bands `<0` expired / `0–30` expiring_soon / `>30` active) on both sides, migrated permit derivations to it, wired `PATCH /api/permits/:id` Edit dialog + OVERDUE/Action-due UI. Full detail in CLAUDE_ARCHIVE.md. Open issue: `/issues` page routed but orphaned from sidebar.
- **2026-06-23 and earlier:** See CLAUDE_ARCHIVE.md.
