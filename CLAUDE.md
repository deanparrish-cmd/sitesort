...
77. **Minimal-by-default Team Portal + retired doc tabs into filtered Shared with me** — new members see only 8 core sections; extra sections gate on granted permissions. **DEPLOYED+prod-verified** (2026-07-20, `main → 91d6d53c`). *(full detail in CLAUDE_ARCHIVE.md)*
78. **Site issue archive/restore + individual photo removal + admin hard delete** — soft-delete/restore for photos+issues, admin-only hard delete. **DEPLOYED** (2026-07-20, `main → 63c2a744`). *(full detail in CLAUDE_ARCHIVE.md)*
79. **Fix stale mirrored contact name on Team tab + portal-invite surname gate** — `lib/person-name.ts#canonicalPersonName` resolves from source instead of a drift-prone mirror. **DEPLOYED, user-confirmed fixed** (2026-07-20, `main → 63c2a744`). *(full detail in CLAUDE_ARCHIVE.md)*
80. **Fix uncaught chunk-load crash after a deploy** — `lazyWithRetry()` self-heals stale lazy-route chunks post-Publish. **DEPLOYED** (2026-07-20, `main → cc5916b1`). *(full detail in CLAUDE_ARCHIVE.md)*
...
- **2026-09-15 (untracked, logged 2026-09-16):** separate Replit Agent session, 9 commits (`e80133d`..`b10667a`), Published 3x. (1) Project detail tabs: merged `buildManagementTabs`+`buildActivityTabs` into one `buildProjectTabs()` with a fixed display order, switched the tab bar from flex-wrap to a responsive grid (`grid-cols-2` mobile / `sm:grid-cols-3` / `xl:grid-cols-6`), capitalized "Close-out"→"Close-Out" and "Check-ins"→"Check-Ins". (2) Added `DictationButton` (voice-to-text, reusing #75's shared component) to Plant & Materials dialog fields (Name/Location/Notes) and the Issues-tab photo-log fields (Description/Zone). (3) New shared "month folder" pattern (`components/ui/month-folder.tsx` — `MonthFolder`/`splitByMonth`/`monthLabel`, comment notes it's intended for check-ins/daily-reports/site-issues too but only notifications page uses it so far): Notifications page now shows the current month's items inline and buckets earlier months into collapsible folder rows; dashboard "Recent Activity" card refactored to share a `renderActivity` helper (behavior unchanged, still shows latest 8). `typecheck` clean, no em-dash/en-dash violations in new copy.
