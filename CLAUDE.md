...
58. Dashboard outstanding-invoices widget — full-width "Outstanding Invoices" card (`pages/dashboard/index.tsx`, between Needs Attention and Active Projects): up to 5 unpaid/overdue invoices (overdue first), `(N)` count, "View all → /invoices". Per-row Open (`/invoices?invoice=<id>`), Share (when `attachmentUrl`), Mark Paid (amber, `canManageInvoices`; PATCHes paid, drops row, opens move-to-project picker if unassigned). Reuses existing `GET /api/invoices` fetch, no new API.
