...
- **2026-07-03 — Team Portal (Feature #61) BUILT, NOT yet deployed:** see #61 for the full feature. STEP-0 investigate-first; PD confirmed "option 1" (dedicated `/api/portal` namespace, one full pass, proposed section mapping). `ensure-schema.ts` migration verified ✓ (ran clean on boot vs real DB). typecheck + both builds green. Backend E2E via curl proved invite→accept→reads→containment-403s→activity-feed→revoke; portal pages browser-clean. Test data cleaned. **Tree has UNCOMMITTED changes — not committed/pushed, awaiting PD review.** NEXT: PD review → commit/push → Publish; then optional F5.
...
- **PD test backlog:** Done+DEPLOYED: B1, B2, F1 P0-3, F2 (close-out), F3 (drawing revisions), F4 (H&S tab). Team Portal (#61) built 2026-07-03 (awaiting deploy). Remaining (confirm scope with PD, per F2–F4 pattern): **F5** Daily Site Reports hub (company-wide aggregate of `daily_reports`), **F6** subbie/merchant docs at setup, **F7** Site Board live on-site count, **F8** Timeline programme link, **F9/F10** spikes.
