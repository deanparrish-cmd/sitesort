...
86. **PIN-based document sign-off (Pending Sign-offs, dashboard + portal)** — every sign-off (all doc types, not just 3) now requires the signer's hashed 4-digit PIN; portal gained view/acknowledge/pin endpoints as twins of the dashboard's; new `pin_audit_log` table; shared `hooks/use-sign-off-flow.ts` state machine. **DEPLOYED+prod-verified** (2026-07-23): both dashboard and portal PIN sign-off flows round-tripped against `www.sitesort.co.uk` (wrong PIN → 401, correct PIN → 200, distribution flips pending→acknowledged, PM sees correct attribution) with throwaway fixtures (cleaned up after). *(full detail in CLAUDE_ARCHIVE.md)*
87. **Activity-entry deep-links + daily reports join Team Portal sharing** — fixed 3 non-clickable-activity gaps (daily report document-activity rows, PM Activity feed's "(opened an item)" placeholder, dashboard/notifications' `document_uploaded` handler) via shared `lib/deep-link.ts`; closed the one real Share-dialog gap (`daily_report` excluded from `ShareModal`'s Team Portal gate) end-to-end, scoped to the site diary only per an explicit user decision. **DEPLOYED+prod-verified** (2026-07-23): `typecheck` clean, `check:layout` 96/96, full functional round-trip + screenshots, confirmed live post-Publish (`df372dd`). *(full detail in CLAUDE_ARCHIVE.md)*
...
90. **Three portal fixes: nav/GET permission gating, explicit site manager, verified submit→view already worked** — **(a)** Site Issues/Plant & Materials/Daily Report nav items and their 5 GET routes weren't actually permission-gated despite #77 claiming they were; fixed. **(b)** Wrong ("Annabelle Rose", a subcontractor) site manager was an unordered `members[0]` fallback with no deliberate designation; added `projects.siteManagerId` (nullable, PM-chosen picker), never guessed — "Not set" when null. **(c)** Reported "can't reopen a submitted item" bug could NOT be reproduced — already works. **DEPLOYED+prod-verified** (2026-07-25): `typecheck` clean, `check:layout` 48/48, full round-trips locally then live against prod with throwaway fixtures, cleaned up after. *(full detail in CLAUDE_ARCHIVE.md)*
