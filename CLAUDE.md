...
89. **Fix dead Overview-tab "Recent Activity" card** — `GET /api/projects/:id` hardcoded `recentActivity: []`; the route now builds a real feed (`getRecentActivity()` in `artifacts/api-server/src/routes/projects.ts`) merging recent document uploads, submitted photo/issue logs, daily site-update notes, and completed milestones, sorted newest-first, capped at 8. Distinct from the manager-only `RecentActivityGlance`/`ProjectTeamActivity` portal-audit widgets, which were already working. **Verified** (2026-07-25): `typecheck` clean, confirmed live via API + browser screenshot (real entries render, no console errors).
90. **Three portal fixes: nav/GET permission gating, explicit site manager, verified submit→view already worked** — **(a)** Site Issues/Plant & Materials/Daily Report nav items and their 5 GET routes weren't actually permission-gated despite #77 claiming they were; fixed (nav filter + `requirePortalPermission` on all 5 GET routes). **(b)** Wrong ("Annabelle Rose", a subcontractor) site manager was an unordered `members[0]` fallback with no deliberate designation anywhere; added `projects.siteManagerId` (nullable, PM-chosen via a new picker in Edit Project Details), never guessed — shows "Not set" when null. **(c)** Reported "can't reopen a submitted item" bug could NOT be reproduced — live-verified (create→submit→note→reopen, all 3 types, API + real portal browser session) and it already works. **Verified** (2026-07-25): `typecheck` clean, `check:layout` 48/48 @360px, full round-trips, fixtures cleaned up. Committed, not yet Published. *(full detail in CLAUDE_ARCHIVE.md)*
...
- **2026-07-25 (2) — shipped #90, three portal fixes requested together** (nav/permission gating, site-manager logic, submit→view flow — see #90). `typecheck` clean, `check:layout` 48/48 @360px. Committed, not Published.
- **2026-07-25 (1) — fixed the recentActivity gap flagged at the end of the 2026-07-23 session:** shipped **#89** — `GET /api/projects/:id` no longer hardcodes `recentActivity: []`; it now aggregates real document/photo/note/milestone events per project. `typecheck` clean, verified via direct API call and a logged-in browser screenshot of the Overview tab (real entries, no console errors). Also noted: local `git pull` fails — `main` and `origin/main` have diverged (92 vs 65 commits), consistent with `origin` being a synthetic API-pushed mirror rather than real shared history; left unresolved pending user decision, use `push-robust.ts` to push.
- **2026-07-23 — full day: corrected stale #84/85/86 status, shipped #87, caught+fixed a gap in a concurrent session's #88, then Published:** opener found the prior session's "not yet committed" note on #84/85/86 was wrong (already Published under `898f8f0`) — corrected CLAUDE.md, prod-verified all three. Shipped **#87** (activity deep-links + daily report portal sharing, two audited fixes) — `typecheck` clean, `check:layout` 96/96, full functional round-trip + screenshots. Found a SEPARATE Replit Agent session building **#88** (configurable per-document PIN sign-off) in this same workspace — confirmed no conflict with #86, found+fixed a missing `ensure-schema.ts` entry that would've broken every prod document endpoint. User Published (`df372dd`, a genuine Deployment) — confirmed #87+#88 both healthy live. End-of-session: re-verified all UI changes at 360px+768px with a targeted click-through check (dialogs need a click to open, beyond `check:layout`'s page-load-only coverage) — all PASS, no overflow. **New issue found, not fixed, flagged for next session**: `GET /api/projects/:id` hardcodes `recentActivity: []` server-side — the Overview tab's "Recent Activity" card (distinct from the working `RecentActivityGlance`/`ProjectTeamActivity` widgets) never shows anything. Pushed to GitHub; working tree clean at session end. *(full detail in CLAUDE_ARCHIVE.md)*
