...
- **RESOLVED 2026-06-23 (session 3):** Live expiry-reminder email send **confirmed on prod** (the item open since session 2). Fired `POST /api/test-email` `{"template":"permit"}` → `200 {"ok":true}` to two inboxes (dean.parrish@me.com + amy-parrish@hotmail.co.uk), both **received**. Proves `RESEND_API_KEY` is set in prod + Resend accepts + the `sendPermitExpiryEmail` template renders/sends end-to-end. Residual (optional, not a code issue): the *scheduled daily job* only emails items in the −6d…+30d window, and all current prod permits/certs are 69–88d expired (`milestoneFor()`→null), so to watch the automated path itself fire, add a permit ~7d out with Responsible Person on a notify-enabled inbox → boot log will show `sent: 1`. Outlook/Hotmail may junk first-domain sends (deliverability/DNS, not code).
