...
export async function generateDailyReportForProject(
  ...
  return db.transaction(async (tx) => {
    ...
    const inserted = await tx
      ...
      .values({ id, projectId, reportDate, checkinCount, documentEventCount, photoCount, data })
      .onConflictDoNothing({ target: [dailyReportsTable.projectId, dailyReportsTable.reportDate] })
      .returning({ id: dailyReportsTable.id });

    if (inserted.length === 0) {
      // A report for this project/date already exists — keep it immutable.
