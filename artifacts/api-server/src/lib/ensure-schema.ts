...
export async function ensureSchema(): Promise<void> {
  try {
    ...
    // Per-document "require PIN sign-off" override (lib/signoff.ts#pinRequiredForDoc):
    // safety-critical types (method_statement/permit/safety) always require a PIN;
    // this lets a PM force it on any other document too. Added here because the
    // column existed in the dev DB (direct drizzle push) but was missing from this
    // boot migration — every documents.* read/write would have 500'd in prod.
    await pool.query(`ALTER TABLE documents ADD COLUMN IF NOT EXISTS require_pin_signoff boolean NOT NULL DEFAULT false`);

    // Deliberate PM-chosen site manager (lib/site-board.ts) — was previously
    // auto-picked (role='manager' member, else first member returned), which
    // could surface a subcontractor. Null = "not set", never guessed.
    await pool.query(`ALTER TABLE projects ADD COLUMN IF NOT EXISTS site_manager_id text REFERENCES users(id)`);

    // Project-level approver authority, grantable to multiple people per
    ...
    // In House Team "Add/Edit Team Member" only had the admin/project_manager/
    // site_worker permission dropdown, with no way to record an actual job
    // title (Site Manager, QS, Labourer, etc.) — it was getting stuffed into
    // Notes and never shown on the card. Same free-text pattern as
    // people.role_title, but on users directly since in-house team members
    // are managed via the users/company_members path, not the people table.
    await pool.query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS role_title text`);

    // Site check-in sign-out (one row per in/out cycle) + photos linked to a daily report.
    // First boot with the sign-out columns: every check-in recorded so far predates
    // sign-out, so mark it 'legacy' (history) instead of leaving hundreds of rows
    // looking like people still on site. Runs once (only when the column is new).
    const hadSignOut = await pool.query(`SELECT 1 FROM information_schema.columns WHERE table_name = 'site_checkins' AND column_name = 'checked_out_at'`);
    await pool.query(`ALTER TABLE site_checkins ADD COLUMN IF NOT EXISTS checked_out_at timestamp`);
    await pool.query(`ALTER TABLE site_checkins ADD COLUMN IF NOT EXISTS checked_out_by text REFERENCES users(id) ON DELETE SET NULL`);
    await pool.query(`ALTER TABLE site_checkins ADD COLUMN IF NOT EXISTS checkout_note text`);
    await pool.query(`ALTER TABLE site_checkins ADD COLUMN IF NOT EXISTS checkout_method text`);
    if (hadSignOut.rowCount === 0) {
      await pool.query(`UPDATE site_checkins SET checkout_method = 'legacy' WHERE checked_out_at IS NULL`);
    }
    await pool.query(`CREATE INDEX IF NOT EXISTS site_checkins_project_open_idx ON site_checkins (project_id, checked_out_at)`);
    await pool.query(`ALTER TABLE photos ADD COLUMN IF NOT EXISTS daily_report_date text`);

    logger.info("ensureSchema: site_checkins sign-out cols + photos.daily_report_date ready + company_members + expiry_reminder_logs + stripe_webhook_events + project_closeouts + documents.revision + daily_notes.photo_url + photos/permits/insurance assignment cols + users email-verification cols + team-portal (users.portal_only, project_members uq, project_invites, activity_log) + people table + project_invites/project_members person_id + daily_notes/daily_reports base tables + daily_reports F5 manager-report cols + portal_shares + portal_sessions + push_subscriptions + pending_pushes + subcontractor_documents + subcontractors/people.archived_at + people.first_name/last_name + subcontractors.contact_first_name/contact_last_name + project_members write-permission cols + activity_log.metadata + photos closure/updated_at cols + plant_items/plant_item_attachments/plant_item_distributions + people.is_primary_contact + person_certifications + primary-contact/project_members backfill + project_members.can_edit_daily_report + messages.project_id + photos archive/photo-removal cols + primary-contact name self-heal ready + photos/daily_reports submitted_at+submitted_by + plant_items portal_draft cols + portal_submission_notes table + submitted-backfill ready + users.platform_admin + Dean/Amy seeded ready + pin_audit_log table ready + documents.require_pin_signoff ready + projects.site_manager_id ready + project_members.is_project_manager ready + accepted-invite/project_members.person_id self-heal ready + failed_stripe_cancellations table ready + users.role_title ready");
  } catch (err) {
