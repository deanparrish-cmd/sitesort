              ...
              <div key={trade} className="bg-card border rounded-xl overflow-hidden shadow-sm">
                ...
                {open && (
                  <div className="border-t divide-y">
                    {members.map(sub => (
                      <div key={sub.id} className={cn("flex items-center gap-4 px-5 py-4 hover:bg-muted/10 transition-colors", sub.paymentHold && "bg-red-50/50 dark:bg-red-950/10")}>
                        {/* Avatar */}
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex flex-wrap gap-x-4 gap-y-0.5 mt-1">
                            {sub.contactEmail && (
                              <a href={`mailto:${sub.contactEmail}`} className="flex items-center gap-1 text-xs text-muted-foreground hover:text-primary transition-colors">
                                <Mail className="w-3 h-3" />{sub.contactEmail}
                              </a>
                            )}
                            {sub.contactPhone && (
                              <a href={`tel:${sub.contactPhone}`} className="flex items-center gap-1 text-xs text-muted-foreground hover:text-primary transition-colors">
