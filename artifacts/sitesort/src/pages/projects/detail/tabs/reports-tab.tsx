        ...
        {caps.isInternal && (
          <TabsContent value="reports">
            ...
            {reports.length === 0 ? (
              ...
              // Current month's reports stay listed; earlier months collapse
              // into one folder each ("July 2026") to keep the tab short.
              const sorted = [...reports].sort((a, b) => b.reportDate.localeCompare(a.reportDate));
              const { current, byMonth } = splitByMonth(sorted, rep => rep.reportDate);
              const reportCard = (rep: typeof sorted[number]) => (
                <Card
                  key={rep.id}
              ...
              return (
                <div className="space-y-2">
                  {current.map(reportCard)}
                  {byMonth.size > 0 && (
                    <>
                      <h4 className="text-sm font-bold text-muted-foreground uppercase tracking-wide pt-4 pb-1">Previous months</h4>
                      {[...byMonth.entries()].map(([key, list]) => (
                        <MonthFolder key={key} label={monthLabel(key)} count={list.length} countLabel="report" testId={`button-report-month-${key}`}>
